...
class ESClient(object):
    ...
    Arguments:
        ...
        http_auth: Authentication details. Can be either a tuple with username/
                   ...
                   this will attempt to create an AWS4Auth auth object,
                   which requires aws access/secret.
        use_ssl: Boolean
    ...
    def from_env(cls, hosts_var='ES_ENDPOINT', http_auth_var='ES_HTTP_AUTH',
        ...
        if 'http_auth' in result_dict:
            try:
                # If we can turn this into a tuple, great. Otherwise just pass
                # along as-is (e.g. if it's 'aws4auth')
                result_dict['http_auth'] = make_tuple(result_dict['http_auth'])
            except ValueError:
                pass
        elif 'http_auth' not in kwargs:
            # Default to 'aws4auth' as http_auth type in the event no
            # ES_HTTP_AUTH found in environment and no http_auth found in kwargs
        ...
        kwarg_names = ['timeout', 'ca_certs', 'client_cert', 'client_key',
