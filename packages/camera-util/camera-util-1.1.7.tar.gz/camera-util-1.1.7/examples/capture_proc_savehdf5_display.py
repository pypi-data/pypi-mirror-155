...
##########################################################################
# Results
# 20-30 frames capertured/s, 1.2-2.2 Cubes generated/s,  2.2 Cubes saved per second
#
##########################################################################
...
flatfield      = np.cast['uint16'](2**8.*np.ones((height, width), 'uint16')) # flatfield correction scaled so that 255=100%
...
while(not stop):
    ...
    if frame_idx >= 14: # 0...13 is populated
        ...
        img = data_cube_corr[0,:,:]
        cv2.putText(img,"Cube:{}".format(num_cubes_generated), textLocation0, font, fontScale, fontColor, lineType)
        cv2.imshow('Proc', img )
