...
def argument_inspector(arg, no_of_arguments=2, p=False, print_width=120):
    from collections import defaultdict
    import warnings
    warnings.filterwarnings("ignore")
    ...
    if no_of_arguments >= 1:
        import enhanced_dir
        for i in enhanced_dir.extended_builtins + inspection_arguments:
            try:
                x = eval(f'{arg}({i})')
                rtrnd_dct[1].add(i)
            except:
    ...
    if no_of_arguments >= 2:
        for i in enhanced_dir.extended_builtins + inspection_arguments:
            for j in enhanced_dir.extended_builtins + inspection_arguments:
                try:
                    x = eval(f'{arg}({i}, {j})')
                    rtrnd_dct[2].add((i, j))
                except:
    ...
    if no_of_arguments >= 3:
        for i in enhanced_dir.extended_builtins + inspection_arguments:
            for j in enhanced_dir.extended_builtins + inspection_arguments:
                for k in enhanced_dir.extended_builtins + inspection_arguments:
                    try:
                        x = eval(f'{arg}({i}, {j}, {k})')
...
inspection_arguments = [1, 1.2, 'abc', '1', '1.2', '1/2', {1, 2, 3}, {'a': 1, 'b': 2}, (1, 2, 3), [1, 2, 3],
                        {'a', 'b', 'c'}, ('a', 'b', 'c'), ['a', 'b', 'c'], 1 + 2j, b'2', b'a', bytearray(1),
