...
class Materialization:
    ...
    def _get_materialization(self):
        query = """
            query Materialization($id: String!) {
                materialization(id: $id) { id, state, path, jobsSubmitted, jobsCompleted }
            }
        """
    ...
    def wait(self) -> str:
        ...
        self._mtr = self._get_materialization()
        if self._mtr["state"] not in ["processing", "materialized"]:
            raise Exception(f"Error in {self}: {self._mtr['state']}")

        while self._mtr["jobsSubmitted"] == 0 and self._mtr["state"] == "processing":
            sleep(0.5)
            self._mtr = self._get_materialization()
