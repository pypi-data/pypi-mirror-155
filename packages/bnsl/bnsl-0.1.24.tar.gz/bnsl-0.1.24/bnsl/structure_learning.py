...
def hc(data, method='standard', score_function='default', alpha=0.3, mul=1, debug=False):
    '''
    ...
    if method == 'lw':
        ...
        cache = {var: {tuple([]): local_score(data, [varnames.index(var)], score_function, alpha=alpha, mul=mul)} for
                 var in varnames}
                ...
                for vj in candidate[vi]:
                    ...
                    if not cyc_flag:
                        ...
                        if par_sea not in cache[vj]:
                            ...
                            cache[vj][par_sea] = local_score(data, cols, score_function, alpha=alpha, mul=mul)
                        time_score += time.time() - start_time
                ...
                for par_vi in dag[vi]['par']:
                    ...
                    if not cyc_flag:
                        ...
                        if par_sea_vi not in cache[vi]:
                            ...
                            cache[vi][par_sea_vi] = local_score(data, cols, score_function, alpha=alpha, mul=mul)
                        time_score += time.time() - start_time
                    ...
                    if par_sea not in cache[vi]:
                        ...
                        cache[vi][par_sea] = local_score(data, cols, score_function, alpha=alpha, mul=mul)
                    time_score += time.time() - start_time
