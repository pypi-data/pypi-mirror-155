...
def nal(data, cols, alpha=0.3, mul=1):
    ...
    if data_local.shape[0] == 0:
        ...
        return nal_counter(data_local, arities, cols, data.shape[0], 0.3 + data_local.shape[0] / data.shape[0] / 10, mul)
...
@njit(fastmath=True)
def nal_counter(data, arities, cols, datasize, alpha=0.3, mul=1):
    strides = np.empty(len(cols), dtype=np.uint32)
    idx = len(cols) - 1
    stride = 1
    while idx > -1:
    ...
    nal /= np.sum(N_ijk)
    if alpha == 1:
        nal -= 0.5 * np.log(datasize) / datasize * (arities[cols[0]] - 1) * strides[0]
    else:
        nal -= mul / data.shape[1] * np.power(datasize, -alpha) * (arities[cols[0]] - 1) * strides[0]
    return nal
