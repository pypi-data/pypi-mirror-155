    ...
    def statusBar(self):
        ...
        if statusbar is None:
            ...
            help_button = SimpleButton(
                icon=icon,
                toolTip='查看帮助', visible=help.isVisible(),
            )
            ...
            def open_url():
                ...
                try:
                    name = re.findall(pattern, self.name)[0]
                    category = re.findall(pattern, self.category)[0]
                except:
                    name = self.name
                    category = self.category
                name = name.replace(' ', '').lower()
                category = category.replace(' ', '').lower()
                base_url = 'https://chengxianzn.one/docs/Orange3'
                # base_url = 'http://localhost:4000/docs/Orange3'
                url = f'{base_url}/{category}/{name}/'.lower()
                if url is not None:
                    QDesktopServices.openUrl(QUrl(url))

            # help_button.clicked.connect(help.trigger)
            help_button.clicked.connect(open_url)
