...
def get(
    ...
    *,
    ...
    for query in queries:
        base_url = get_api_base_url_v2()
        observations = _make_api_request(
            f"{base_url}/data/get",
            access_token=access_token,
            params=query,
        )
        ...
        observation_json = _make_api_request(
            f"{base_url}/observation_json/get",
            access_token=access_token,
            params=query,
        )
        if len(observation_json):
            observation_json = pd.DataFrame.from_dict(observation_json)

            observation_json[
                ["series_id1", "series_id2", "price_area"]
            ] = observation_json["series_id"].str.split("/", 2, expand=True)
            observation_json["series_id"] = (
                observation_json["series_id1"] + "/" + observation_json["series_id2"]
            )
            observation_json = observation_json.drop(
                ["series_id1", "series_id2"], axis=1
            )
            is_intraday_trades = observation_json["series_id"] == "NP/IntradayTrades"

            def agg_func(x):
                return list(x)

            observation_data_intraday = observation_json[is_intraday_trades]
            ...
            observation_json = observation_json[~is_intraday_trades]
            if len(observation_json):
                observation_json = observation_json.sort_values(
                    by=["from_dt", "version"]
                ).pivot_table(
                    values="value",
                    columns=["series_id", "price_area"],
                ...
                observation_json = pd.DataFrame()

            observation_json = observation_json.merge(
