...
def get_blur(type):
    if type =='motion':
        kernel = np.load('data/blur/kernel_m19.npy')
        k1,k2 = kernel.shape
...
class spectra():
    ...
    def space(self, img, space='adrgb', wp=None, xyz=False):
        ...
        if space.lower() == 'xyz':
            ...
            return img @ np.load('data/SRF/NikonD700.npy')
        else:
...
class spectra_metric():
    ...
    def __init__(self,x1,x2,scale =16):
        self.scale = scale
        self.info = x1.shape
        if len(self.info) == 3:
    ...
    def get_Evaluation(self, l=1, k1=0.01, k2=0.03):
        return self.PSNR(),self.SAM(),self.ERGAS(),self.SSIM(k1=k1, k2=k2, l=l),self.MSE()

    def Evaluation(self,idx=0,l=1,k1=0.01,k2=0.03):
        PSNR,SAM,ERGAS,SSIM,MSE = self.get_Evaluation(l,k1,k2)
