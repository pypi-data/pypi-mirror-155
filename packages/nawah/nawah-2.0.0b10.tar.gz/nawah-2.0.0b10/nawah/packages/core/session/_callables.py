...
async def _auth(
    ...
    user_query = Query(pipe=[{key: {"$eq": doc[key]}}], special={"$limit": 1})
...
async def _reauth(env: "NawahEnv", query: "Query") -> "Results":
    ...
    session_query = Query(pipe=[{"_id": {"$eq": query["_id:$eq"][0]}}])
    ...
    if results["args"]["docs"][0]["expiry"] < datetime.datetime.utcnow().isoformat():
        results = await call(
            ...
            env=env,
            query=Query(pipe=[{"_id": {"$eq": env["session"]["_id"]}}]),
        )
    ...
    await call(
        ...
        skip_events=[Event.PERM],
        env=env,
        query=Query(pipe=[{"_id": {"$eq": results["args"]["docs"][0]["user"]}}]),
    ...
    await call(
        ...
        skip_events=[Event.PERM],
        env=env,
        query=Query(pipe=[{"_id": {"$eq": results["args"]["docs"][0]["_id"]}}]),
        doc={
            "expiry": (
                datetime.datetime.utcnow() + datetime.timedelta(days=30)
            ).isoformat()
    ...
    user_results = await call(
        ...
        skip_events=[Event.PERM],
        env=env,
        query=Query(pipe=[{"_id": {"$eq": results["args"]["docs"][0]["user"]["_id"]}}]),
    )
    results["args"]["docs"][0]["user"] = user_results["args"]["docs"][0]
