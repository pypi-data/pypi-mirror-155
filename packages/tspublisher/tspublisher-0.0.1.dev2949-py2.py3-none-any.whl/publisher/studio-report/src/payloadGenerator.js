...
const processInfoCards = (sections, title, withTitle = true) => {
      ...
      .map((section, sectionIndex) => {
        return section.body
          .map((body, bodyIndex) => {
            ...
            const articleTitle =
              withTitle && bodyIndex === 0 && sectionIndex === 0 ? title : '';
            const subtitle = bodyIndex === 0 ? section.title : '';
            ...
            switch (type) {
              ...
              case componentTypes.LIST: {
                return article({
                  title: articleTitle,
                  subtitle: subtitle,
                  list: body.list || body.numbered_list,
                ...
                if (path(['author', 'image', 'name'], body)) {
                  return [
                    article({
                      title: articleTitle,
                      subtitle: subtitle,
                      text: body.author.name,
                      gallery: [{ src: body.author.image.img_src }],
                    }),
                    article({
                      text: body.author.desc,
                    }),
                  ];
                } else {
                  return article({
...
const generateSection = (infoCardList, withTitle = true) =>
  infoCardList.map(({ information_card: { sections, title } }) =>
    processInfoCards(sections, title, withTitle)
  );
      ...
      return objective.steps.map((step) => {
        ...
        const content = step.widgets
          .map((widget) => {
            if (widget.text) {
              if (widget.text.information_cards) {
                const infoCards = widget.text.information_cards.map(
                  ({ information_card: { sections, title } }) =>
                    processInfoCards(sections, title)
                );
              ...
              return article({ title: 'text', text: widget.text.content });
            }
            if (widget.mcq) {
              return [
                article({ title: 'Question', text: widget.mcq.content }),
                article({
                  title: 'Options',
                  list: widget.mcq.choices.concat(widget.mcq.answer),
                }),
                article({ title: 'Correct answer', text: widget.mcq.answer }),
              ];
            }
            if (widget.gallery) {
        ...
        const sideContentWithLabels = !labelsList.length
          ...
          : flattenedContents.concat(
              article({ title: 'Labels', list: labelsList, list_type: 'A' })
            );
              ...
              (acc, infoCard) => {
                if (
                  ...
                  const orderedTitle = `${String.fromCharCode(
                    charOffset + index
                  )}. ${infoCard.information_card.title}`;
                  return flatten(
                    acc.concat(
                      processInfoCards(
                        infoCard.information_card.sections,
...
const generatePhaseSection = (phasesArray, isExtractMode) => {
  return phasesArray.map((phase, phaseNumber) => {
    const learnHeader = pill({
      text: `Phase ${phaseNumber + 1} - ${phase.name}`,
      mode: 'Learn mode',
      width: '100%',
    ...
    const phaseInfo = `Phase ${phaseNumber + 1} - ${phase.name}`;
...
const generateTableOfContents = (overviewList, devicesList, phasesList) => {
  const sections = [
    {
      title: 'Overview',
      contents: overviewList.map((infoCard) => ({
        text: `${infoCard.information_card.title}`,
        ...
        devicesList.map((deviceInfoCard) =>
          deviceInfoCard.information_card.sections
            ? deviceInfoCard.information_card.sections
                ...
                .filter(Boolean)
            : {
                text: deviceInfoCard.information_card.title,
                link: makeLink({
    ...
    },
    {
      ...
      contents: phasesList.map((phase, i) => ({
        ...
        subsections: [
          {
            text: 'Learn mode',
            link: makeLink({ name: phase.name, mode: 'Learn', anchor: true }),
          },
          {
            text: 'Test mode',
            link: makeLink({ name: phase.name, mode: 'Test', anchor: true }),
          },
        ],
      })),
    },
...
const generateReviewDocument = (yamlJson, consts, isExtractMode) => {
  const phaseSection = generatePhaseSection(yamlJson.phases, isExtractMode);
  const devicesSection = generateSection(yamlJson.devices, false);
  const overviewSection = generateSection(yamlJson.overview);
  ...
  const dateString = `${getRevisionDate()}`;
  return {
