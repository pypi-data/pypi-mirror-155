...
def visualize_predictions_and_ground_truth(fpath_eval_set, fpath_coco_predictions, materials_dict, conf_thresh = None):
    ...
    for image in images:
        ...
        for pred in predictions:
            if pred["image_id"] == img_id:
                ...
                x, y = bbox[:2]
                plt.text(x, y, f"{confidence:.2f}  {materials_dict[material_id]}", color="white", backgroundcolor=(0.01, 0.01, 0.01, 0.1))
...
def get_predictions_per_gt(gt_json, inference_json, iou_threshold=0.5, conf_threshold=None, plot_hist=False):
    """Dataset-wide"""
    ground_truths = ds_annotations.load_json(gt_json)
    predictions = ds_annotations.load_json(inference_json)
...
if __name__ == "__main__":
    ...
    # # Local training
    # yolo_fdir = "/media/findux/DATA/Documents/Malta_II/datasets/dataset_09_mini_yolo/"
    # run_training(yolo_fdir, "COCO-Detection/faster_rcnn_R_50_FPN_1x.yaml",
    #              output_dir="/home/findux/Desktop/tests/local_trainings/ds_09/",
    #              device="cpu", imgs_per_batch=2, iterations=500,
    #              rpn_batch_size_per_img=128, roi_heads_batch_size_per_img=128)
