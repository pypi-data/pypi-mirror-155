        ...
        for db in datamodel["databases"].values():
            for table in db["tables"].values():
                for field in table["fields"].values():
                    ...
                    semantic_type = field.get("semantic_type", None)
                    if semantic_type is not None:
                        update_attrs["semantic_type"] = semantic_type
    ...
    def db_data(self, db):
        ...
        for table in db["tables"]:
            ...
            for field in table["fields"]:
                if field["semantic_type"] == "type/FK":
...
#                             # Now it's not necessary as we export all snippets earlier, and the id mapping is handled by Mapper class.
    ...
    def deref_fields(self, expression):
        if isinstance(expression, list):
            if len(expression) == 3 and expression[0] == "field" and isinstance(expression[1], int):
                expression[1] = self.mappings["fields"][expression[1]]
                self.deref_fields(expression[2])
            elif len(expression) == 2 and expression[0] == "segment":
            ...
            if "source-field" in expression:
                expression["source-field"] = self.mappings["fields"][expression["source-field"]]
            elif "default" in expression:
                expression["default"] = self.deref_fields(expression["default"])
        return expression
