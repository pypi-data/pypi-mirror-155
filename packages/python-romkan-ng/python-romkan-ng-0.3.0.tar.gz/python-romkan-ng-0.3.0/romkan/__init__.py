...
def _kanpat_cmp(x, y):
    return (len(y) > len(x)) - (len(y) < len(x)) or (
...
def to_kunrei(str):
    """
    Convert a Kana (仮名) or a Hepburn Romaji (ヘボン式ローマ字)
    to a Kunrei-shiki Romaji (訓令式ローマ字).
    """
    ...
    # If unmodified, it's a Hepburn Romaji Romaji
    # -- convert it to a Kunrei-shiki Romaji
    # If modified, it's also a Hepburn Romaji Romaji
    # -- convert it to a Kunrei-shiki Romaji
    tmp = tmp.lower()
    tmp = normalize_double_n(tmp)
