...
def read_udfs(pickleSer, infile, eval_type):
    ...
    if eval_type in (PythonEvalType.SQL_SCALAR_PANDAS_UDF,
        ...
        pandas_zero_conf_conversion_enabled = runner_conf.get(
            "spark.databricks.execution.pandasZeroConfConversion.enabled", "true") \
            .lower() == "true"
        pandas_zero_conf_conversion_max_bytes = runner_conf.get(
            "spark.databricks.execution.pandasZeroConfConversion.maxBytesPerSlice",
            (256 * 1024 * 1024) - (256 * 1024))
        pandas_zero_conf_conversion_max_bytes = int(pandas_zero_conf_conversion_max_bytes)
        should_use_edge = \
            pandas_zero_conf_conversion_enabled and pandas_zero_conf_conversion_max_bytes > 0
        ...
        if eval_type == PythonEvalType.SQL_COGROUPED_MAP_PANDAS_UDF and should_use_edge:
            ser = DatabricksCogroupUDFSerializer(
                ...
                max_bytes=pandas_zero_conf_conversion_max_bytes)
            ...
            if notebook_path != "" and (notebook_path.startswith("/Repos/") or
                ...
                for i in range(len(sys.path)):
                    ...
                    if sys.path[i] == "/WSFS_NOTEBOOK_DIR":
                        ...
                        # /Repos/<user@email>/<repo>, split that on /
                        # produces empty string as first element
                        parts = notebook_path.split('/', 4)
                        if len(parts) == 5:
                            repo_path = prefix + "/".join(parts[0:4])
