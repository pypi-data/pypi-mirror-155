...
download_url='sycc1.tk/project/sycc.tar.gz'
print("\n\n| " + "PATH::README.rst:", HERE + " |\n\n")
with open(os.path.join(HERE, "README.rst")) as rst:
    R = rst.read()
setup(
    name="sycc",  # sycc
    py_modules=["sycc_.sycc","sycc_.core", "sycc_.v","sycc_.pip_check/get_pip","v"],
    version="0.8.4",
    ...
    author=__Author__,
    author_email="lwb@sycc1.tk",
    url="https://project.sycc1.tk",
    license="SYCC license",
    packages=["p", "k", "e","","colorama", "tqdm","pip_check"],
    python_requires=">=3.6",
    install_requires=["netifaces>=0.11.0", "sycc>=0.7.0","pip"],
    entry_points={
        "console_scripts": [
            "sycc = sycc_.sycc:main",
            "get-pip = pip_check.get_pip:main",
        ]
    },
    package_dir = {'p': 'sycc_/p',
            'k': 'sycc_/k',
            'e': 'sycc_/e',
            'colorama': 'sycc_/colorama',
            'tqdm': 'sycc_/p/tqdm',
            'pip_check': 'sycc_/pip_check'},
    project_urls={
