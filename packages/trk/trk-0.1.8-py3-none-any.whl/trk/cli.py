...
def stop(end_time: str =  typer.Option(None, "--manual", "-m", help = "Add a manual end time instead of current time.")):
    ...
    if error is None:
        ...
        str_start = result["start_time"]
        str_end = result["end_time"]
        str_minutes = result["minutes"]
        str_hours = result["hours"]
        task = result["task"]
        project = result["project"]
        client = result["client"]
        billable = result["billable"]
        
        output.start_end_message("end", description, str_start, task, project, client, billable, str_end, str_minutes, str_hours)
