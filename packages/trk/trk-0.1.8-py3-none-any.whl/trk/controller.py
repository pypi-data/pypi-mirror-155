...
class Tracker:
    ...
    def _parse_active_information(self,positions, lines):
        ...
        client = self._get_task_value(lines[active_task["client"]["position"]].strip())
    ...
    def _calculate_hours_from_minutes(self, minutes):
        hours = str(round(int(float(minutes))/60, 2))
        return(hours)
    ...
    def stop(self, end_time):
        ...
        if  status["active_task_count"] > 1:
            ...
            error = "Error: No Tasks Have Been Started"
            return({"error" : error}) 
        else:            
            error = None
            ...
            hours = self._calculate_hours_from_minutes(minutes)
            print(hours)
