...
def start_end_message(type, description, str_start, task, project, client, billable, str_end = None ,str_minutes = None, str_hours = None ):

    if type == "start":
        prompt = "Start: "
        message_start = typer.style(prompt, fg = typer.colors.BRIGHT_GREEN)
    if type == "end":
        prompt = "Stop: "
        message_start = typer.style(prompt, fg = typer.colors.BRIGHT_RED)
    ...
    if all(v is not None for v in [str_end, str_minutes]):
        # duration
        duration = typer.style("duration: ",  fg = typer.colors.BRIGHT_YELLOW)
        minutes = typer.style(str_minutes + " minutes",  fg = typer.colors.WHITE)
        between_min_hours = typer.style(" / ",  fg = typer.colors.WHITE)
        hours = typer.style(str_hours + " hours",  fg = typer.colors.WHITE)
        ...
        typer.echo("\t" + duration + minutes + between_min_hours + hours)
        typer.echo("\t" + project_1 + project_2 )
        typer.echo("\t" + client_1 + client_2 )
        typer.echo("\t" + billable_1 + billable_2 + "\n")
