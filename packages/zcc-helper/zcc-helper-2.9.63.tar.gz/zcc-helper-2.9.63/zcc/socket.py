...
class ControlPointSocket:
    ...
    async def connect(self):
        ...
        try:
            ...
            self.logger.error("connect() awating _listen cancellation")
            await self.listen_task
            self.logger.error("connect() received _listen cancellation")
        ...
        while True:
            try:
                ...
                if self.sock and self.sock.fileno() != -1 and self.reader:
                    ...
                    self.logger.error(
                        '__listen() socket failure after readline() timeout')
                    break
