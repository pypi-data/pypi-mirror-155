...
from flowhigh.domain import MatchRecognize
from flowhigh.util.Visitor import Visitor
...
class ApiConverter:
    ...
    def __init__(self, visitor: Visitor = None) -> None:
        ...
        self.raw_input: Union[str, None] = None
    ...
    def convert_statement(self, values) -> Statement:
        ...
        if values['_type'] == 'statement':
            ...
            if "rawInput" in values:
                self.raw_input = values['rawInput']
            ...
            # after visiting statement - remove raw input
            self.raw_input = None
    ...
    def convert_ds(self, values) -> Ds:
        ...
        if "ds" == values['_type']:
            ...
            if "_id" in values:
                _id = values['_id']
                raw_input = self.get_raw_input_by_id(_id)
            ...
            ds_obj = Ds(value, alias, direction, _id, raw_input, out_obj, in_obj, match_recognize, table_sample,
                        _filter, agg,
                        vagg,
                        sort,
                        page, set_op,
                        ds_type, name, refdb, refds, refsch, fullref, _refTo)
    ...
    def convert_vagg(self, vagg_json) -> Vagg:
        ...
        if vagg_json["_type"] == "vagg":
            ...
            if "vfilter" in vagg_json:
                vfilter = self.convert_vfilter(vagg_json["vfilter"])
            vagg = Vagg(vfilter)
    ...
    def convert_op(self, op_json) -> Op:
        ...
        if op_json['_type'] == "op":
            ...
            op = Op(_expr.value, _expr.alias, _expr.direction, _id, raw_input, expr, op_type)
    ...
    def convert_expr(self, expr_json) -> Expr:
        ...
        if expr_json["_type"] == "asterisk":
            return self.convert_asterisk(expr_json)
    ...
    def convert_case(self, case_json) -> Case:
        ...
        if case_json['_type'] == "case":
            ...
            case = Case(_expr.value, _expr.alias, _expr.direction, _expr.id, _expr.rawInput, expr, when, else_obj)
    ...
    def convert_when(self, when_json) -> When:
        ...
        if when_json['_type'] == "when":
            ...
            when = When(_expr.value, _expr.alias, _expr.direction, _expr.id, _expr.rawInput, expr, then)
    ...
    def convert_then(self, then_json) -> Then:
        ...
        if then_json['_type'] == "then":
            ...
            then = Then(_expr.value, _expr.alias, _expr.direction, _expr.id, _expr.rawInput, expr)
    ...
    def convert_attr(self, attr_json) -> Attr:
        ...
        if attr_json['_type'] == "attr":
            ...
            _attr = Attr(value, alias, direction, _id, raw_input, refvar, refatt, refds, refdb, refsch, fullref,
                         refoutidx)
    ...
    def convert_asterisk(self, asterisk_json) -> Asterisk:
        """
        Converts json to Attr Node
        @param asterisk_json: JSON
        @return: Attr
        """
        if asterisk_json['_type'] == "asterisk":
            refvar = ""
            refatt = ""
            refds = ""
            refdb = ""
            ...
            raw_input = ""
            _id = ""
            if "refvar" in asterisk_json:
                refvar = asterisk_json['refvar']
            if "refatt" in asterisk_json:
                refatt = asterisk_json['refatt']
            if "refds" in asterisk_json:
                refds = asterisk_json['refds']
            if "refdb" in asterisk_json:
                refdb = asterisk_json['refdb']
            if "refsch" in asterisk_json:
                refsch = asterisk_json['refsch']
            if "fullref" in asterisk_json:
                fullref = asterisk_json['fullref']
            if "refoutidx" in asterisk_json:
                refoutidx = asterisk_json['refoutidx']

            if "value" in asterisk_json:
                value = asterisk_json['value']

            if "alias" in asterisk_json:
                alias = asterisk_json['alias']

            if "direction" in asterisk_json:
                direction = asterisk_json['direction']

            # if "rawInput" in attr_json:
            #     raw_input = attr_json['rawInput']
            if "_id" in asterisk_json:
                _id = asterisk_json['_id']
                raw_input = self.get_raw_input_by_id(_id)

            exprs: List[Expr] = []
            if "_value" in asterisk_json:
                for value in asterisk_json["_value"]:
                    exprs.append(self.convert_expr(value))

            _asterisk = Asterisk(value, alias, direction, _id, raw_input, refvar,
                                 refatt, refds, refdb, refsch, fullref, refoutidx, exprs)
            _asterisk.visitor = self.visitor
            _asterisk._json_dict = asterisk_json
            return _asterisk
        else:
            raise Exception("Invalid Asterisk")
    ...
    def convert_const(self, const_json) -> Const:
        ...
        if const_json['_type'] == "const":
            ...
            const = Const(_expr.value, _expr.alias, _expr.direction, _expr.id, _expr.rawInput)
    ...
    def convert_cast(self, cast_json) -> Cast:
        ...
        if cast_json['_type'] == "cast":
            ...
            cast = Cast(_expr.value, _expr.alias, _expr.direction, _expr.id, _expr.rawInput, expr, data_type)
    ...
    def convert_frame(self, frame_json) -> Frame:
        ...
        if frame_json['_type'] == "frame":
            ...
            frame = Frame(_expr.value, _expr.alias, _expr.direction, _expr.id, _expr.rawInput, expr, _type, lower_limit,
                          upper_limit,
    ...
    def convert_func(self, func_json) -> Func:
        ...
        if func_json["_type"] == "func":
            ...
            func = Func(_expr.value, _expr.alias, _expr.direction, _expr.id, _expr.rawInput, expr, within_group,
                        func_type, name)
    ...
    def convert_insert(self, insert_json) -> Insert:
        ...
        if insert_json["_type"] == "insert":
            ...
            insert = Insert(_expr.value, _expr.alias, _expr.direction, _expr.id, _expr.rawInput, insert_type,
                            is_overwrite, is_else,
    ...
    def convert_join(self, join_json) -> Join:
        ...
        if join_json['_type'] == "join":
            ...
            join = Join(_expr.value, _expr.alias, _expr.direction, _expr.id, _expr.rawInput, ds, op, join_type,
                        sub_type)
    ...
    def convert_merge(self, merge_json) -> Merge:
        ...
        if merge_json["_type"] == "merge":
            ...
            merge = Merge(_expr.value, _expr.alias, _expr.direction, _expr.id, _expr.rawInput, target, source,
                          condition, actions)
    ...
    def convert_multivalue(self, multivalue_json) -> MultiValue:
        ...
        if multivalue_json["_type"] == "multivalue":
            ...
            multivalue = MultiValue(_expr.value, _expr.alias, _expr.direction, _expr.id, _expr.rawInput, expressions)
    ...
    def convert_rotate(self, rotate_json) -> Rotate:
        ...
        if rotate_json["_type"] == "rotate":
            ...
            rotate = Rotate(_expr.value, _expr.alias, _expr.direction, _expr.id, _expr.rawInput, rotate_type, aggregate,
                            pivot_column,
    ...
    def convert_structref(self, structref_json) -> StructRef:
        ...
        if structref_json["_type"] == "structRef":
            ...
            structref = StructRef(_expr.value, _expr.alias, _expr.direction, _expr.id, _expr.rawInput, column_ref,
                                  structure_ref)
    ...
    def convert_tablefunc(self, tablefunc_json) -> TableFunc:
        ...
        if tablefunc_json["_type"] == "tablefunc":
            ...
            tablefunc = TableFunc(_expr.value, _expr.alias, _expr.direction, _expr.id, _expr.rawInput, tablefunc_type,
    ...
    def convert_update(self, update_json) -> Update:
        ...
        if update_json["_type"] == "update":
            ...
            update = Update(_expr.value, _expr.alias, _expr.direction, _expr.id, _expr.rawInput, target, in_obj, assign,
                            _filter)
    ...
    def convert_wfunc(self, wfunc_json) -> Wfunc:
        ...
        if wfunc_json["_type"] == "wfunc":
            ...
            wfunc = Wfunc(_expr.value, _expr.alias, _expr.direction, _expr.id, _expr.rawInput, expr, partition, sort,
                          frame, func_name)
    ...
    def convert_current(self, current_json) -> Current:
        ...
        if current_json["_type"] == "current":
            ...
            current = Current(_expr.value, _expr.alias, _expr.direction, _expr.id, _expr.rawInput, current_type)
    ...
    def convert_inlinetable(self, inlinetable_json) -> InlineTable:
        ...
        if inlinetable_json['_type'] == "inlinetable":
            ...
            inline_table = InlineTable(_expr.value, _expr.alias, _expr.direction, _expr.id, _expr.rawInput, expr)
    ...
    def convert_queryingstage(self, queryingstage_json) -> QueryingStage:
        ...
        if queryingstage_json['_type'] == "queryingstage":
            ...
            if "_id" in queryingstage_json:
                _id = queryingstage_json['_id']
                raw_input = self.get_raw_input_by_id(_id)

            queryingstage = QueryingStage(value, alias, direction, _id, raw_input, location, file_format, pattern)
    ...
    def convert_tablesample(self, tablesample_json) -> TableSample:
        ...
        if tablesample_json['_type'] == "tablesample":
            ...
            if "_id" in tablesample_json:
                _id = tablesample_json['_id']
                raw_input = self.get_raw_input_by_id(_id)

            table_sample = TableSample(value, alias, direction, _id, raw_input, sample_method, sample_type, probability,
                                       num, seed_type, seed)
    ...
    def convert_match_recognize(self, match_recognize_json) -> MatchRecognize:
        ...
        if match_recognize_json['_type'] == "matchrecognize":
            ...
            if "_id" in match_recognize_json:
                _id = match_recognize_json['_id']
                raw_input = self.get_raw_input_by_id(_id)

            match_recognize = MatchRecognize(value, alias, direction, _id, raw_input, partition_by, order_by, measure,
    ...
    def _create_expr(self, values):
        ...
        return Expr(value, alias, direction, _id, raw_input)

    def get_raw_input_by_id(self, _id: str):
        if self.raw_input is None:
            return ""

        # extract positions from id
        start, end = _id.split("-")
        if start is None or end is None:
            return ""
        start = int(start)
        end = int(end)
        return self.raw_input[start:end]
