...
class LineageVisitor(BaseVisitor):
    ...
    def visit_ds(self, obj: Ds):
        ...
        if obj._id not in self.ds_scope_id.keys():
            self.ds_scope_id[obj._id] = []

        if obj._id not in self.ds_scope.keys():
            self.ds_scope[obj._id] = get_ds_scope(obj)

        # enter object, save global pointer to local variable and move global pointer to current object
        self.id_to_ds[obj._id] = obj
        ...
        if prev_ds is not None:
            ...
            if obj._refTo is None:
                self.ds_scope_id[prev_ds._id].append(obj)
            else:
                ref_obj = self.id_to_ds[obj._refTo]
                self.ds_scope_id[prev_ds._id].append(ref_obj)
