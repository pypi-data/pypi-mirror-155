...
class Photometry:
    ...
    def start(self, nbOfStars, center = True, maxVal = 30000):
    ...
    def toGif(self, path, starPassage = False ):
        ...
        if starPassage:
            listOfStarsPassages = np.concatenate(self.StarPassages())
        ...
        def animate_func(i):
            ...
            if starPassage:
                starsPassages = self.detector.correctStars(listOfStarsPassages, i, 1)
            ...
            for j, pos in enumerate(self.Appertur[i].positions):
                ...
                if j >= self.Appertur[i].idxOfStars or not starPassage:
                    col = 'black'
                ...
                if starPassage:
                    for star in starsPassages:
                        if Utils.dist(star, pos) < 3*self.getFwhm(i):
                            col = 'red'
                            break
...
if __name__ == '__main__':
    ...
    rbt = 'C:/Users/antoi/OneDrive/Documents/PHD/lightcurve/entrainement/21-03-19RBT/'
    suh = 'C:\\Users\\antoi\\OneDrive\\Documents\PHD\\lightcurve\\entrainement\\Suh/19-04-01Suh/Arequipa/'
    
    seq = glob.glob(rbt + "*Met*.f*t*")
    # seq = [seq[0], seq[73],seq[-1]]
    dark = glob.glob(rbt + "*Dark*.f*t*")
    flat = glob.glob(rbt + "*flat_R_1x1*.f*t*")
    bias = glob.glob(rbt + "*bias_1x1*.f*t*")
