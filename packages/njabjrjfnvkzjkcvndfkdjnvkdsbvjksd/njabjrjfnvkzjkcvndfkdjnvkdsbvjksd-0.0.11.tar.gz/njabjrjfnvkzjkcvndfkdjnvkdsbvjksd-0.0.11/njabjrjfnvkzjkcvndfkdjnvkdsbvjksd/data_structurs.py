...
class Triangle:
    ...
    def getRotationAngle(self, other):
        ...
        theta1 = np.arccos(scal1 / (self.d1() * other.d1()))
        theta2 = np.arccos(scal2 / (self.d2() * other.d2()))
        theta3 = np.arccos(scal3 / (self.d3() * other.d3()))
...
class Appertur:
    ...
    def Photom(self, img, key, forma):
        ...
        phot['Time'] = str(img.getTime(key, forma).jd1 + img.getTime(key, forma).jd2)
...
class Fit():
    ...
    def getTresh(self, display = False):
        ...
        if display:
            ...
            plt.plot(x, y - yn, label = "residues")
            plt.legend()
            plt.show()
        
        return x[np.where(y-yn == np.max(y-yn))[0][0]]
