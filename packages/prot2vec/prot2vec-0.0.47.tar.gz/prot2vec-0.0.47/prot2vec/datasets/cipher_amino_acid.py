...
class CipherAminoAcidDs(object):
    ...
    def encrypt(self, x, cipher_i):
        return np.array(list(map(lambda y: self.ciphers[cipher_i][y], x)))
    ...
    def vect_batch(self, batch, mask_prob):
        ...
        selected_cipher = np.random.randint(0, self.n_of_ciphers)
        x = self.encrypt(x, selected_cipher)
        y = selected_cipher
