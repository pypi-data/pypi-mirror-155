...
from gnomix.paths import DATA_FOLDER, TEST_DATA_FOLDER, DEMO_DATA_FOLDER

CHR = "22"
SEQUENCE_DATA_FILE = DATA_FOLDER / "1000genomes" / f"ALL.chr{CHR}.phase3_shapeit2_mvncall_integrated_v5a.20130502.genotypes.vcf.gz"
SAMPLE_MAP_FILE    = DATA_FOLDER / "1000genomes" / "1000g.smap"
SEED = 94305
...
def generate_dataset(name):
    ...
    assert os.path.exists(SEQUENCE_DATA_FILE), f"Sequencing data {SEQUENCE_DATA_FILE} not found."
    ...
    if all(dataset_files_exist):
        print(f"data at {dataset_folder} exists! skipping...")
    else:
