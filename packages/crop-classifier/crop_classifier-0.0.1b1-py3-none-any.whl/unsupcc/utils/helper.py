...
def get_tile_shape(aoi_shp, bbox):
    aoi_tile_shp = 'sentinel2_bbox_tile.shp'
    try:
        os.remove(aoi_tile_shp)
    except OSError:
        pass
    if aoi_shp:
        call(str('ogr2ogr -clipsrc ' + aoi_shp + ' ' + aoi_tile_shp + ' ' +
                 'unsupcc/satellite/satellite_tiles/s2_tile.shp'), shell=True)
    else:
        min_x, min_y, max_x, max_y = bbox[0], bbox[1], bbox[2], bbox[3]
        call(
            str('ogr2ogr -f "ESRI Shapefile" ' + aoi_tile_shp + ' ' + 'unsupcc/satellite/satellite_tiles/s2_tile.shp'
