import requests
from vgmdb.utils import SafeEtree
...
class Artist:
    ...
    def getDiscography(self):
        discography_list = self.tree.xpath('discography', final=False)
        albums_all = []
...
if __name__ == '__main__':
    import numpy as np
    import time
    test_id = list(range(1, 10000))
    np.random.shuffle(test_id)
    bad_example = []
    for k in test_id[:50]:
        if k in bad_example:
            continue
        print("current artist id：", k)
        try:
            print(Artist(k, proxies=7890).albums)
        except Exception:
            bad_example.append(k)
            print("获取失败，请手动检查，id=", k)
        time.sleep(12)
    print(bad_example)
