        ...
        try:
            ...
            if 'unzip' in kwargs.keys() and kwargs.get('unzip'):
                if 'newfilename' in kwargs.keys():
                    ...
                    newfilename = None
                if 'newdirname' in kwargs.keys():
                    newdirname = kwargs.get('newdirname')
                else:
                    newdirname = None
                # if 'newfilename' in kwargs.keys():
                ...
                #     unZip(filename=download_filename, newdirname=kwargs.get('newdirname'),
                #           dst_path=kwargs.get('dst_path'))
                # else:
                unZip(filename=download_filename, newfilename=newfilename, newdirname=newdirname,
                      dst_path=kwargs.get('dst_path'))
            if 'del_src' in kwargs.keys() and kwargs.get('del_src') and path.isfile(download_filename):
...
def jdk_install(version: Optional[str] = 'latest', dest_dir: Optional[str] = '.'):
    ...
    mirror: https://mirrors.tuna.tsinghua.edu.cn/Adoptium/{version}/jdk/x64/{os}/
    '''
    latest = getWebList(url='https://mirrors.tuna.tsinghua.edu.cn/Adoptium/', match='title="[0-9]+"')
    version = latest[0] if len(latest)>0 and version == 'latest' else version
    os = get_osname()
    if os == 'windows':
        url = f'https://mirrors.tuna.tsinghua.edu.cn/Adoptium/{version}/jdk/x64/{os}/'
        ...
        url = f'https://mirrors.tuna.tsinghua.edu.cn/Adoptium/{version}/jdk/x64/linux/'
        filename = getWebList(url, match='href="OpenJDK(.+?).tar.gz"', flags=0)
        if len(filename) > 0:
            url = f'{url}OpenJDK{filename[0]}.tar.gz'
            download(url=url, filename=f'jdk{version}_{os}_64.tar.gz', newdirname=f'jdk{version}_{os}_64', dst_path=dest_dir, unzip=True, del_src=True)
...
def allure_install(version: Optional[str] = 'latest', dest_dir: Optional[str] = '.'):
    ...
    name = get_osname()
    if name == 'windows':
        # download(src_list[2] + filename, filename=filename, newdirname=f'apache-jmeter-{version}', dst_path=dest_dir,
        ...
        download(url=f'https://repo.maven.apache.org/maven2/io/qameta/allure/allure-commandline/{version}/allure-commandline-{version}.zip',
                 filename=f'allure-{version}.zip', dst_dir=dest_dir, newdirname=f'allure-{version}', unzip=True, del_src=True)
    else:
        download(url=f'https://repo.maven.apache.org/maven2/io/qameta/allure/allure-commandline/{version}/allure-commandline-{version}.tgz',
                 filename=f'allure-{version}.tgz', dst_dir=dest_dir, newdirname=f'allure-{version}', unzip=True, del_src=True)
