...
def _fast_torchvision(func, root=None, train=False, transform=None, size=None, download=False, **kwargs):
    """
    ...
    if root is None:
        try:
            return func(root='~/.pipetorchuser', download=False, train=train, transform=transform, **kwargs)
        except: pass
        try:
            return func(root='~/.pipetorch', download=False, train=train, transform=transform, **kwargs)
        except: pass
        root = '~/.pipetorchuser'
    return func(root=root, download=download, train=train, transform=transform, **kwargs)
...
class _FastFashionMNIST(FashionMNIST):
    def __init__(self, root=None, train=True, transform=None, size=None, **kwargs):
        super().__init__(root=root, train=train, **kwargs)
        self.transform=transform
        if size is not None:
            self.data = F.interpolate(self.data, (size, size))
        self.data = self.data.float().div(128).sub(1).unsqueeze(dim=1)
...
def FastFashionMNIST(root=None, train=True, transform=None, size=None, download=False, **kwargs):
...
def _fast_databunch(func, root=None, test=False, valid_perc=0.1, batch_size=64, transform=None, size=None, 
    ...
    '''
    train_ds = func(root, transform=transform, train=True, size=size, **kwargs)
    if test:
        test_ds = func(root, transform=transform, train=False, size=size, **kwargs)
    else:
        test_ds = None
...
def mnist(root=None, test=False, valid_perc=0.1, batch_size=64, transform=None, size=None, 
    ...
    return _fast_databunch(FastMNIST, root=root, test=test, valid_perc=valid_perc, batch_size=batch_size, 
                           transform=transform, size=size, normalize=normalize, num_workers=num_workers, 
                           **kwargs)
...
def fashionmnist(root=None, batch_size=64, test=False, valid_perc=0.1, size=None, transform=None, 
          num_workers=2, **kwargs):
    return _fast_databunch(FastFashionMNIST, root=root, test=test, valid_perc=valid_perc, batch_size=batch_size, 
                           transform=transform, size=size, num_workers=num_workers, 
                           **kwargs)
