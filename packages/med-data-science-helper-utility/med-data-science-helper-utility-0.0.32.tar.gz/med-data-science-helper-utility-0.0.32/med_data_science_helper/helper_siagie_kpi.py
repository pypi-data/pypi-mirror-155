...
def generar_kpis_p_deser_by_dist(df,anio_df=None ,cache=False):
    
    anio_df_menos_1 = anio_df-1
    
    df_p_deser = get_p_desercion_by_distrito(anio=anio_df_menos_1,macro_region="Peru",cache=cache)
    
    ho.print_items(df_p_deser.columns, excepto=["D_DIST"])
    
    df = pd.merge(df, df_p_deser ,left_on="D_DIST",  right_on="D_DIST", how='left')
    
    df.rename({"P_DESERCION_D_DIST": "P_DESERCION_D_DIST_T_MENOS_1"}, axis=1, inplace=True)
...
def get_p_desercion_by_distrito(anio=None, macro_region=None,modalidad=None,id_grado_list=None,cache=False ):
    ...
    filename = 'p_desercion_by_dist'
    ho.print_message('agregar_kpi_p_desercion_by_dist')
    ...
    df_siagie = hadb.get_siagie_por_anio(anio,macro_region=macro_region,desercion=True,  modalidad=modalidad , id_grado_list= [6,7,8] ,columns_n= ['ID_PERSONA','COD_MOD','ANEXO'])
