...
from yrocr.resources.post_process.biz_license_three_in_one.base_biz_license import treat_unified_social_credit_code
...
def post_process(image, result, txt_result, use_show_name):
    ...
    result['负责人'] = treat_lawer(result['负责人'], left_text)
    result['经营范围'] = treat_business_scope(result['经营范围'], left_text)

    # 右侧字段：成立日期、营业期限、营业场所
    result['成立日期'] = treat_found_date(result['成立日期'], full_text)
    result['营业期限'] = treat_period(result['营业期限'], full_text, right_text)
    result['营业场所'] = treat_addr(result['营业场所'], full_text)
...
def treat_found_date(text, right_text):
    """
    成立日期
    右侧顺序为：成立日期、营业期限、营业场所、发证日期
    因此，可能存在两个日期，必须从 【营业期限、营业场所】两个进行切割
    :param text:
    ...
    if len(found_date) == 0:
        # 得到前半段全文，并使用正则匹配
        part_full_text = find_pre_full_text_by_key(right_text, ['营业期限', '营业场所'])
...
def treat_period(text, full_text, right_text):
    ...
    if right_text.__contains__('营业期限') and right_text.__contains__('营业场所'):
        peroid = get_text_between_start_end_key(right_text, '营业期限', '营业场所')
        return peroid
...
def treat_issue_date(text, right_text, txt_result):
    """
    发证日期
    :param text:
    :param full_text: 右侧全文
    :param txt_result: 原始OCR识别结果
    :return:
    """
    # 先去除特殊字符
    remove_reg = '[\'!\"#$%&()*+,-./:;<=>?@，。?★、…【】《》？“”^！[\\]_\{\|\}（）：~\s]+'
    ...
    if len(issue_date) == 0:
        # 得到前半段全文，并使用正则匹配
        suffix_full_text = find_suffix_full_text_by_key(right_text, ['营业场所', '场所'])
        issue_date = treat_by_reg(suffix_full_text, reg)
        if len(issue_date) > 0:
            return issue_date
        # 由于证书的发证日期中的年、月、日三个数字经常是有上下高度差歪曲的。识别后的文字可能为："年05、月27日2020" 这样的
        # 此时，直接从原始的txt_result
