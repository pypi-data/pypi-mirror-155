...
def astracarta(**kwargs):
    ...
    if kwargs.get("shape") is not None:
        if kwargs.get("shape") != "circle":
            if kwargs.get("shape") != "square":
                raise ValueError('-shape may only be "circle" or "square".')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-shape', dest='shape', action='store', const=None, default=None, type=str, help='OPTIONAL: Shape of field to query: "square" (default) or "circle". Circle may only be used if pixwidth and pixheight are equal. Square query uses a polygon query with corners defined by an ad-hoc WCS given the supplied field parameters, whereas circle uses a radius.')
    parser.add_argument('-rotation', dest='rotation', action='store', const=None, default=None, type=float, help='OPTIONAL: Field rotation, applicable to a square query. Raises an exception if used for a circular query.')
