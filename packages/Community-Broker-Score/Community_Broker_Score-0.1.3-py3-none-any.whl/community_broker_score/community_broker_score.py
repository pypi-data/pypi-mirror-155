...
def detect_brokering_edges(nodes, edges):
    ...
    """
    #rename dataframe with columns A and B 
    ...
    edges_A_with_cid = edges_for_calculation.merge(nodes, left_on="A", right_on="id", how="inner").drop(columns=["id"])
...
def detect_community_brokers(nodes, edges):
    ...
    #rename dataframes with columns A and B 
    edges_for_calculation = edges.rename(columns={f'{edges.columns[0]}':'A', f'{edges.columns[1]}':'B'})
