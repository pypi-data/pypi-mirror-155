...
def LastNDistinctAgg(col1, col2, n):
    ...
    if is_spark3():
        ...
        return Column(_f(_to_seq(sc, [col1, col2, n], _to_java_column)))
...
def LimitedListConcatAgg(col1, n):
    ...
    if is_spark3():
        ...
        return Column(_f(_to_seq(sc, [col1, n], _to_java_column)))
...
def _make_lastn_partial(n: int):
    ...
    def _lastn_partial_spark3(col):
        return [LastNDistinctAgg("timestamp", col, n)]

    return _lastn_partial_spark3 if is_spark3() else _lastn_partial


def _make_lastn_full(n: int):
    def _lastn_full(columns, window):
        col = LimitedListConcatAgg(columns[0], functions.lit(n)).over(window)
        return col

    def _lastn_full_spark3(column_name, window):
        col = LimitedListConcatAgg(column_name, n).over(window)
        return col

    return _lastn_full_spark3 if is_spark3() else _lastn_full
