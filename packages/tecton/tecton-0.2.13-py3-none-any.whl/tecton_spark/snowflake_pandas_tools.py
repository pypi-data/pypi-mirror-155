#
# Snowflake provided this hacky fix(a modified write_pandas function), but it may break at some point
...
def write_pandas(
    ...
    if compression not in compression_map.keys():
        raise ProgrammingError(
            "Invalid compression '{}', only acceptable values are: {}".format(compression, compression_map.keys())
        )
    ...
    while True:
        try:
            ...
            create_stage_sql = (
                "create temporary stage /* Python:snowflake.connector.pandas_tools.write_pandas() */ " '"{stage_name}"'
