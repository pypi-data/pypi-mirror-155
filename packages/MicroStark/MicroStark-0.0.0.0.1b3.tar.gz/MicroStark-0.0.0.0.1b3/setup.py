...
with open("stark/constants.py", "r", encoding="utf-8") as f:
    text = f.read()
    pat = r"['\"]([^'\"]+)['\"]"
...
def get_packages():
    return [path.replace("/", ".") for path, _, _ in os.walk("stark") if "__" not in path]
...
setup(
    ...
    url='https://github.com/StarkBotsIndustries',
