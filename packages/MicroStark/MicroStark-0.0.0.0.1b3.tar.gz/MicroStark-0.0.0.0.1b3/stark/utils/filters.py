...
def get_filters(
    ...
    if isinstance(query, list):
        cmd_filter = f.create(lambda _, __, query_: getattr(query_, string).lower() in query and getattr(query_, string).lower() not in negate)
    elif isinstance(query, str):
        query = query.lower()
        if not startswith:
            cmd_filter = f.create(lambda _, __, query_: getattr(query_, string).lower() == query and getattr(query_, string).lower() not in negate)
        else:
            cmd_filter = f.create(lambda _, __, query_: getattr(query_, string).lower().startswith(query) and getattr(query_, string).lower() not in negate)
    elif not query:
        cmd_filter = None
    else:
