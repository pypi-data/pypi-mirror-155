...
class JslSecrets:
    ...
    def from_jsl_home(log=True) -> Union['JslSecrets', bool]:
        jsl_creds_file = f'{expanduser("~")}/.johnsnowlabs/jsl_credentials.json'
        if os.path.exists(jsl_creds_file):
            if log:
                print('Loading secrets from JSL home')
            json_dict = JslSecrets.json_path_as_dict(jsl_creds_file)
    ...
    def store_in_jsl_home(secrets: 'JslSecrets') -> None:
        jsl_creds_path = f'{expanduser("~")}/.johnsnowlabs'
        jsl_creds_file = f'{jsl_creds_path}/jsl_credentials.json'
        if os.path.exists(jsl_creds_file):
            return

        if not os.path.exists(jsl_creds_path):
            os.makedirs(jsl_creds_path)

        with open(jsl_creds_file, 'w') as json_file:
            json.dump(secrets.__dict__, json_file)
        print('Stored secrets in JSL home 📋')
