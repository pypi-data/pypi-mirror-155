...
global countdownpause, optimgain, optimexpo, optim_histo
countdownpause = 5
optimgain = -1
optimexpo = -1
optim_histo = 50 # we try middle of the range in %
...
def disp_mutext(lena, wrapped_text_o):
    ...
    #img = np.clip(img, 0, None ) # for dark img values, clip to 0
...
in progress"""
                ...
                if (not frame is None) and (rpi_name!="") \
                   and  (key == ord('o')):
                    ...
                    #post_response = requests.post(url=post_addr, data=post_data)
                    countdown = [0., 0.001, 0.002, 0.004, 0.008, 0.01, 0.02, 0.04, 0.08, 0.1, 0.2, 0.4, 0.8, 1.0, -2, 0., 0.001, 0.002, 0.004, 0.008, 0.01, 0.02, 0.04, 0.08, 0.1, 0.2, 0.4, 0.8, 1.0, -98, -99]
                    now = datetime.datetime.now()
                    delta = (now - datetime.datetime(1970,1,1))
                        ...
                        if 'countdown_ana_on' in locals():
                            if countdown_ana_on:
                                ...
                                if countdown[0]<-97:
                                    ...
                                    if np.abs(resy - optim_histo).argmin() >len(resx)/2:
                                        optimgain = 1.
                                        print("i... BUT USE GAIN 1 !")
                                    print( "RESULTS:", resx,resy )
