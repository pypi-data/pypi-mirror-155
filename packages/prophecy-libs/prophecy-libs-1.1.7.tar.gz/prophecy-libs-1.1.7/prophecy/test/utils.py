...
def convertStringValueToDatatype(dataType: DataType, value: str):
    if isinstance(dataType, StringType):
        ...
        newValue = 0 if isNull(value) else str.encode(value)
        ...
        newValue = bytearray(0) if isNull(value) else bytearray(value)
    elif isinstance(dataType, BooleanType):
        newValue = False if isNull(value) else (value == "true")
    elif isinstance(dataType, DateType):
        newValue = datetime.now() if isNull(value) else datetime.strptime(value, '%Y-%m-%d')
    elif isinstance(dataType, TimestampType):
        newValue = datetime.now() if isNull(value) else datetime.strptime(value, '%Y-%m-%dT%H:%M:%S.%f%z')
    elif isinstance(dataType, DecimalType):
        newValue = DecimalType(0) if isNull(value) else float(value)
    else:
