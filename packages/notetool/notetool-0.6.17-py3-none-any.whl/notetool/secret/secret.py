...
from notedrive.sqlalchemy import BaseTable, meta
...
class SecretManage(BaseTable):
    ...
    def select_all(self):
        sql = select(self.table.columns.cate1, self.table.columns.cate2, self.table.columns.cate3,
                     self.table.columns.cate4, self.table.columns.cate5, self.table.columns.value, )
        return [line for line in self.engine.execute(sql)]
...
def save_secret_str(path='~/.secret/secret_str'):
    ...
    for line in manage.select_all():
        r1 = []
        for i, key in enumerate(['cate1', 'cate2', 'cate3', 'cate4', 'cate5', 'value']):
