...
from ..messages import rpc, get_current_timestamp
...
# class HeartBeat:
#   def __init__(self, job_proto: Job, every = 1):
#     self.job_proto = job_proto
#     self.every = every

#   def start(self):
#     self.job_proto.status = Job.ACTIVE
...
#       rpc(
#         nbox_grpc_stub.UpdateRun,
#         UpdateRunRequest(token = self.job_proto.id, job = self.job_proto, updated_at = get_current_timestamp()),
#         "Heartbeat failed",
#         raise_on_error = False
#       )
#       sleep(self.every)
