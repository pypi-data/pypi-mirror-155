...
def run():
	...
	if st.button("submit") : #essay != st.session_state.get(f'rid-{rid}:tid-{tid}:uid-{uid}:essay',"")
		st.session_state[f'rid-{rid}:tid-{tid}:uid-{uid}:essay'] = essay 
		id = redis.r.xadd(xname, {"rid": rid, "tid": tid,"uid": uid, "essay_or_snts": essay, "submit": 1, 'uname': st.experimental_get_query_params().get('uname', [''])[0]})
		if 'debug' in app_state: 
			st.sidebar.write( f"essay submitted {id}" ) 
			st.sidebar.write( redis.r.xrevrange(xname, count=1))
