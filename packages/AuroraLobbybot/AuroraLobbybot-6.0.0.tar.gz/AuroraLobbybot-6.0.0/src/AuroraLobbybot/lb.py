...
class MyClient(fortnitepy.Client):
    ...
    async def event_ready(self):
        ...
        with open(my_file, 'r') as file:
                content = file.read()
                for friend in self.friends:
                    if friend.display_name in content:
                        return
                    else:
    ...
    async def event_friend_request(self, IncommingPendingFriend):
            ...
            with open(my_file, 'r') as file:
                content = file.read()
                if IncommingPendingFriend.display_name in content:
                    return
                else:
                    with open(my_file ,"w", encoding="utf-8") as f:
                        f.write(f"{IncommingPendingFriend.display_name} + ({IncommingPendingFriend.id})\n")
