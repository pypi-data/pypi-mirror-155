...
class Rectifier(ElementCompound):
    ''' Diode Rectifier Bridge
        ...
        Args:
            ...
            labels: Labels to draw on each resistor
            inlabels: Labels to draw on each resistor, inside the rectifier
        ...
        Anchors:
            ...
            * W
    '''
    def __init__(self, fill: bool=False, labels: Sequence[str]=None,
                 inlabels: Sequence[str]=None,
                 labelsize: float=None, inlabelsize: float=None,
                 **kwargs):
        super().__init__(**kwargs)
        D1 = elm.Diode(fill=fill).theta(45)
        D2 = elm.Diode(fill=fill).theta(-45)
        D3 = elm.Diode(fill=fill).theta(-135).reverse()
        D4 = elm.Diode(fill=fill).theta(135).reverse()

        locs = ['top', 'top', 'bottom', 'bottom']
        if labels:
            for label, d, loc in zip(labels, [D1, D2, D3, D4], locs):
                d.label(label, loc, fontsize=labelsize)
                print(labelsize)
        if inlabels:
            for label, d, loc in zip(inlabels, [D1, D2, D3, D4], locs[::-1]):
                d.label(label, loc, fontsize=inlabelsize)
        ...
        self.add(D3)
        self.add(D4)    
        self.anchors['W'] = D1.start
        self.anchors['N'] = D2.start
        self.anchors['E'] = D3.start
        self.anchors['S'] = D4.start
        self.params['theta'] = 0