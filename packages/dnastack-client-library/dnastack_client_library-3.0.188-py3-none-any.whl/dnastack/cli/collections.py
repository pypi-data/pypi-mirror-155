...
from dnastack.cli.helpers.exporter import normalize, to_json, display_result_iterator
...
def _switch_to_data_connect(collection_service_client: CollectionServiceClient,
                            collection_id_or_slug_name: Optional[str] = None) -> DataConnectClient:
...
def list_collections(endpoint_id: Optional[str], no_auth: bool = False):
    """ List collections """
    click.echo(to_json(normalize(_get(endpoint_id).list_collections(no_auth=no_auth))))
...
def list_items(endpoint_id: Optional[str],
    ...
    data_connect_client = _switch_to_data_connect(collection_service_client, actual_collection.slugName)
...
def query_collection(endpoint_id: Optional[str],
                     ...
                     no_auth: bool = False):
    """ Query data """
    client = _switch_to_data_connect(_get(endpoint_id), collection)
