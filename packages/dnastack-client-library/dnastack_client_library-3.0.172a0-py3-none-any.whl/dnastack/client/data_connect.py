...
class DataConnectClient(BaseServiceClient):
    ...
    def iterate_tables(self, no_auth: bool = False) -> Iterator[Table]:
    ...
    def list_tables(self, no_auth: bool = False) -> List[Table]:
        """ List all tables """
        return [t for t in self.iterate_tables(no_auth=no_auth)]

    def table(self, table: Union[Table, TableWrapper, str], no_auth: bool = False) -> TableWrapper:
        """ Get the table wrapper """
        table_name = self._get_table_name(table)
        table_url = urljoin(self.url, f'table/{table_name}/')
        return TableWrapper(http_session=self.create_http_session(no_auth=no_auth),
                            table_name=table_name,
                            url=table_url)

    def get_table(self, table: Union[Table, TableWrapper, str], no_auth: bool = False) -> Table:
        """ Get the table metadata """
        self._logger.warning(
            'This method will be deprecated in v3.1. Please use <client>.table(table_name).info instead.'
        )
        return self.table(table, no_auth=no_auth).info
