...
class Blob(AbstractContextManager):
    ...
    def get_object(self) -> DrsObject:
        ...
        if self.__object:
            ...
            try:
                object_info_response = self.__session.get(
                    urljoin(self.__metadata.drs_server_url,
                            f'objects/{self.__metadata.object_id}')
                )
            ...
            self._logger.debug(f'Responded with HTTP {object_info_status_code}:\n\n{object_info_response.text}\n')
