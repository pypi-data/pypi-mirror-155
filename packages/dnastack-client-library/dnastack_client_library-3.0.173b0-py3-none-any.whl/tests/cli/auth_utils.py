...
def handle_device_code_flow(cmd: List[str], email: str, token: str) -> str:
    ...
    assert exit_code == 0, f'Unexpected exit code:\nSTDOUT:\n{output}\nERROR:\n{error_output}'
...
def _confirm_device_code(device_code_url, email: str, token: str, allow=True):
    ...
    try:
        ...
        driver.quit()
        raise UnexpectedLoginError(f'Failed to log in with {email} at {device_code_url} due to JavaScript error')
    except:
        driver.quit()
        raise UnexpectedLoginError(f'Failed to log in with {email} at {device_code_url} due to unexpected error')
