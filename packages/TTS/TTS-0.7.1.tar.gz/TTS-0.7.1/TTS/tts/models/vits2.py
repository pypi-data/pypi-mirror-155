...
from TTS.tts.layers.losses import ForwardSumLoss
...
class Vits(BaseTTS):
    ...
    def forward_mas(self, outputs, alignment_soft, x, x_mask, y_mask, g, lang_emb):
        # find the alignment path
        attn_mask = torch.unsqueeze(x_mask, 2) * torch.unsqueeze(y_mask, -1)
        attn = maximum_path(alignment_soft, attn_mask.squeeze(1).contiguous()).unsqueeze(1)
    ...
    def _forward_aligner(
        ...
        return alignment_soft, alignment_logprob, attn_mask
    ...
    def forward(  # pylint: disable=dangerous-default-value
        ...
        x_emb, x, m_p, logs_p, x_mask = self.text_encoder(x, x_lengths, lang_emb=lang_emb)
        ...
        # alignment network
        alignment_soft, alignment_logprob, _ = self._forward_aligner(x_emb, y.transpose(1, 2), x_mask, y_mask)
        alignment_soft = alignment_soft.transpose(1, 2)

        # duration predictor
        outputs, attn = self.forward_mas(outputs, alignment_soft, x, x_mask, y_mask, g=g, lang_emb=lang_emb)
        ...
        outputs.update(
            {
                ...
                "alignment_mas": attn.squeeze(1),  # [B, 1, T_de, T_en] -> [B, T_en, T_de]
    ...
    def inference(
        ...
        _, x, m_p, logs_p, x_mask = self.text_encoder(x, x_lengths, lang_emb=lang_emb)
        ...
        if optimizer_idx == 1:
            ...
            with autocast(enabled=False):  # use float32 for the criterion
                loss_dict = criterion[optimizer_idx](
                    ...
                    decoder_output_lens=spec_lengths,
                    input_lens=token_lenghts,
                    alignment_logprob=self.model_outputs_cache["alignment_logprob"],
