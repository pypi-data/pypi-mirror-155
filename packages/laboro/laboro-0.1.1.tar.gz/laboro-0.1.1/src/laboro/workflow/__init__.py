...
class Workflow:
  ...
  def _is_runnable(self, obj):
    if "when" in obj.keys():
      return Processor().process(self.ctx, str(obj["when"]))
    return True

  def _get_iterable(self, obj):
    return Processor().process(self.ctx, str(obj["loop"]))

  def _interpolate(self, obj):
    if "args" in obj:
      interpolated = obj["args"].copy()
  ...
  def _do_action(self, step_name, action):
    ...
    cls_args = self._interpolate(action["object"])
  ...
  def _do_method(self, action_name, instance, instance_name, method):
    ...
    if not self._is_runnable(method):
      ...
      return
    method_args = self._interpolate(method)
