...
class SingleODAutoCalibrationTab:
    def __init__(self, main_od_autocalibration_gui, vial_number):
        ...
        self.run_button = widgets.Button(description="start", tooltip="start serial dilution in vial %d" % vial_number)
    ...
    def handle_milk_volume_input(self, input):
        with self.output:
            ...
            estimated_od = 150 * initial_milk_fraction
    ...
    def run(self, button):
        try:
            with self.main_od_autocalibration_gui.output:
                ...
                for dilution_number in range(15):
                ...
                with self.main_od_autocalibration_gui.plot_output:
                    ...
                    for v in range(1, self.vial_number + 1):
...
class ODAutoCalibrationTab:
    def __init__(self, device):
        ...
        self.text_test = widgets.HTML("""
                        ...
                        In the experiment tab, set the culture type of each vial (e.g. Blank Culture, but not None)
                        ...
                        10. Record the milk concentration of the collected samples and the measured OD values. We can calculate all OD values from this data. GUI for this coming soon.<br><br>
                        
                        To start the measurement in all vials, press all the buttons. The measurements will be done sequentially. To stop a measurement, use the notebook stop button, then stop the pumps and stirrers in the device control tab if necessary.
                        """)
        ...
        self.header = VBox([self.text_test, HBox([self.vial_volume, self.added_milk]), self.output])
        ...
        self.widget = VBox([self.header, self.vial_widgets, self.plot_output],layout=Layout(width="650px"))
