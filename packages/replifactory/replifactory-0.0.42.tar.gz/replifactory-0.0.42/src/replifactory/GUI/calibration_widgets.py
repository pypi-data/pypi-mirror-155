...
class MilkODCorrectorTab:
    def __init__(self, device):
        self.device = device
        self.convert_button = widgets.Button(description="convert", tooltip="convert milk_concentration:signal(mV) calibration data into OD:signal(mV) format and build calibration curves")
        ...
        self.output = widgets.Output()
        self.widget = VBox([self.milk_to_ods_input, HBox([self.clear_button, self.convert_button]), self.output])
    ...
    def fit_milk_ods(self, button):
        with self.output:
            ...
            for v in range(1, 7):
                ...
                ods = dev.od_sensors[v]
                ods.plot_calibration_curve()
