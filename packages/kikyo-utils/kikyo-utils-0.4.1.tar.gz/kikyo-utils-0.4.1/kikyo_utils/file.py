...
def pdf_to_image(data: bytes, limit: Optional[int] = None, format: str = 'jpeg', **params):
    ...
    for i, page in enumerate(pdf):
        # 将每一页pdf读取为图片
        img: Pixmap = page.getPixmap(matrix=mat)
...
def doc_to_pdf(source: bytes, filename: str = None, timeout: int = 20) -> Optional[bytes]:
    ...
    if filename:
        ...
        _meta = inspect_file(source)
        _in = False
        for k, v in content_file_ext.items():
            if _meta.content_type == v:
                data = (f'data.{k}', io.BytesIO(source), v)
                break
