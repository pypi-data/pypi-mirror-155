...
class BuildTranslations(build_py):
    def run(self):
        ...
        if command is None:
            ...
            sys.exit(1)
        translation_source_dir = os.path.join("jdEolConverter", "translations")
        translation_bin_dir = os.path.join(self.build_lib, "jdEolConverter", "translations")
...
setup(name="jdEolConverter",
    ...
    download_url="https://gitlab.com/JakobDev/jdEolConverter/-/releases",
    python_requires=">=3.9",
    include_package_data=True,
    install_requires=[
        "PyQt6"
    ],
    packages=["jdEolConverter"],
    entry_points={
        "console_scripts": ["jdeolconverter = jdEolConverter:main"]
    },
    cmdclass={
    ...
    project_urls={
        "Issue tracker": "https://gitlab.com/JakobDev/jdEolConverter/-/issues",
    },
    classifiers=[
