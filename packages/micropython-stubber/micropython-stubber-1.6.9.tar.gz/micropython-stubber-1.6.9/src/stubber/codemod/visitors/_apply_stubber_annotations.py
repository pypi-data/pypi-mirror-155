...
class TypeCollector(cst.CSTVisitor):
    ...
    def _import_parameter_annotations(self, parameters: cst.Parameters) -> cst.Parameters:
...
class ApplyStubberAnnotationsVisitor(ContextAwareTransformer):  # lgtm [py/multiple-calls-to-init]
    ...
    :class:`~libcst.codemod.visitors.ApplyStubberAnnotationsVisitor` and then call the static
    :meth:`~libcst.codemod.visitors.ApplyStubberAnnotationsVisitor.store_stub_in_context` method,
    ...
    def __init__(
        ...
        # Qualifier for storing the canonical name of the current function.
        self.qualifier: List[str] = []
        self.annotations: Annotations = Annotations() if annotations is None else annotations
    ...
    def store_stub_in_context(
        ...
        """
        context.scratch[ApplyStubberAnnotationsVisitor.CONTEXT_KEY] = (
            stub,
    ...
    def _update_parameters(self, annotations: FunctionAnnotation, updated_node: cst.FunctionDef) -> cst.Parameters:
        ...
        def update_annotation(parameters: Sequence[cst.Param], annotations: Sequence[cst.Param]) -> List[cst.Param]:
            ...
            if self.force and self.overwrite_existing_annotations:
                ## if force & overwrite - then replace parames from source
    ...
    def leave_FunctionDef(self, original_node: cst.FunctionDef, updated_node: cst.FunctionDef) -> cst.FunctionDef:
        ...
        if key in self.annotations.function_annotations:
            ...
            if self.overwrite_existing_annotations or not updated_node.returns:
                updated_node = updated_node.with_changes(returns=function_annotation.returns)
