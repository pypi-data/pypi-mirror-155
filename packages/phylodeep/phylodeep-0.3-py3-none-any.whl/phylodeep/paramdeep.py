...
def paramdeep(tree_file, proba_sampling, model=BD, vector_representation=FULL, ci_computation=False, **kvargs):
    ...
    if tree_size == HUGE:
        ...
        for subtree in extract_clusters(tree, min_size=MIN_TREE_SIZE_SMALL if model != BDSS else MIN_TREE_SIZE_LARGE,
            ...
            subtree_predictions = _paramdeep_tree(subtree, subtree_size, model, proba_sampling, vector_representation,
                                                  ci_computation)
            subtree_size = len(subtree)
            subtree_predictions['tips'] = subtree_size
            predictions = predictions.append(subtree_predictions)
            sizes.append(subtree_size)
        df = pd.DataFrame(columns=[_ for _ in predictions.columns if 'tips' != _])
        sizes = np.array(sizes)
        squared_sizes = np.power(sizes, 2)
        n = sum(sizes)
        pred_val_col = PREDICTED_VALUE if ci_computation else next(iter(predictions.index))
        if 'output' in kvargs:
            predictions.to_csv(kvargs['output'] + '.subtree_predictions.csv')
        for col in df.columns:
            ...
            if ci_computation:
                ...
                ci_2_5 = np.power(np.power(ci_2_5, 2).dot(squared_sizes), 0.5) / n
                ci_97_5 = np.power(np.power(ci_97_5, 2).dot(squared_sizes), 0.5) / n
