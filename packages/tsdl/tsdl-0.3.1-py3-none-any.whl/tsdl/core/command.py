...
def diagnose(context: Context, expression: str, *args, **kwargs):
    ...
    :return:
    """
    condition = expression.format(*args)
    if eval(condition):
        logger.info('~ @DIAGNOSE expression:{} result:{}'.format(condition, True))
        send(context, {'app:show': {
            'msg': 'DIAGNOSE Condition[{}], execute success.'.format(condition) if kwargs.get(
                'success') is None else kwargs.get('success')}})
    else:
        logger.info('~ @DIAGNOSE expression:{} result:{}'.format(condition, False))
        send(context, {'app:show': {
            'msg': 'DIAGNOSE Condition[{}], execute fail.'.format(condition) if kwargs.get(
...
def presume(context: Context, expression: str, *args, **kwargs):
    ...
    condition = expression.format(*args)
    if eval(condition):
        logger.info('~ @PRESUME expression:{} result:{}'.format(condition, True))
        send(context, {'app:show': {
            'msg': 'PRESUME Condition[{}], execute success.'.format(condition) if kwargs.get(
                'success') is None else kwargs.get('success')}})
    else:
        logger.info('~ @PRESUME expression:{} result:{}'.format(condition, False))
        send(context, {'app:show': {
            'msg': 'PRESUME Condition[{}], execute fail.'.format(condition) if kwargs.get(
                'fail') is None else kwargs.get('fail')}})
