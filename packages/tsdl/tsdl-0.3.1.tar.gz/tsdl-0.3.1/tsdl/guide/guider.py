...
def handle_encode_frames(context: Context, encode_frame_parse: dict, **session):
    """
    处理组帧
    """
    parses = {}
    for pos, meter in context.meters.items():
        ...
        for k, v in session:
            parses[pos] = util.replace(parses[pos],
                                       k=eval('context.meters.get(pos).session.{}'.format(v.lower().replace('_', '.'))))
