...
class AzureBackend(AbstractBackend):
    ...
    def get_refs(self, repository_name: str, project: str) -> List[str]:
        # Fill in with your personal access token and org URL

        organization_url = "https://dev.azure.com/energinet"

        # Create a connection to the org
        connection = Connection(base_url=organization_url, creds=self._credential)

        git_client = connection.clients.get_git_client()
        refs_response = git_client.get_refs(repository_name, project=project)

        if refs_response is not None:
            # Get name property of values in the response
            return [ref.name for ref in refs_response.value]
