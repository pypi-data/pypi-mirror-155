...
def parse_pipeline_repo_version() -> Callable:
    ...
    def _parse_pipeline_repo_version(
        ...
        try:
            refs = backend.get_refs(repository_name="pipelines", project="AnalyticsOps")

            # Get all tags in the format "refs/tags/<tag_name>".
            tags = [ref.split("/")[-1] for ref in refs if ref.startswith("refs/tags/")]

            # Remove old tags without SemVer format.
            tags = [tag for tag in tags if re.match(r"v[0-9]+\.[0-9]+\.[0-9]+", tag)]

            # Tags have the format v<tag_name>. Sort by newest SemVer.
            tags.sort(key=lambda t: tuple(map(int, t[1:].split("."))), reverse=True)

            newest_version = tags[0] if len(tags) > 0 else default_version

        except Exception:
            click.echo("Could not get pipeline repo tags. Using default version.")
            newest_version = default_version
