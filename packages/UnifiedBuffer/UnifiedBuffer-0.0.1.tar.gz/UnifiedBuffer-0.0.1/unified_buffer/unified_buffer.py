...
class UnifiedBuffer:
    ...
    def update(self, update_no, data: List[Tuple[str, str, int, datetime]]):
        """
        :param update_no:
        :param data: List of [env_name, phase_name, green_time, time]
        :return: None
        """
        assert len(data) > 0
        conn = sqlite3.connect(self.db_path)
        c = conn.cursor()
        c.executemany('INSERT INTO Actions(EnvName,Phase,GreenTime,Time) VALUES(?,?,?,?);', data)
        c.execute('INSERT INTO Updates(EnvName, No, Time) VALUES(?,?,?);', (self.env_name, update_no, datetime.now()))
        conn.commit()
        conn.close()

    def get(self, n_latest_data: int, phase_name: str) -> List:
        """
        ...
        c = conn.cursor()
        c.execute(
            "SELECT Phase,GreenTime FROM Actions WHERE Phase=? AND EnvName=? ORDER BY Time DESC LIMIT ?;",
            (phase_name, self.env_name, n_latest_data))
        rows = c.fetchall()
        conn.commit()
        conn.close()
        return rows
