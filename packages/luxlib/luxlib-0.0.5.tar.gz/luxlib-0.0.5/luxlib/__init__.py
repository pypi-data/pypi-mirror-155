...
def is_a_file(dir_fname):
    """
    Test si le chemin ramene vers un fichier
...
def get_all_file(start_dir: str) -> list[dict or str]:
    """
    Renvoie tous les fichiers et les dossiers

    param:
    start_dir: string

    return type: list[dict or str]
    """
...
def generate_downloader(start_dir: str, output_dir: str, name="downloader.lxf", max_recursion=1000) -> None:
...
def init_server_sftp(ip_server: str, user: str, password: str, dir_name: str) -> None:
    """
    Permet d'initialiser le serveur distant avec les fichiers et dossier généré

    param:
    ip_server: string
    user: string ('' si pas d'authentification)
    password: string (''si pas d'authetification)
    dir_name: string
...
class Downloader:
    def __init__(self, url_server: str, name_folder: str, custom_java: bool, logger=False, sftp=False, user='', password='', main_dir='', max_recursion=1000) -> None:
        """
        Constructeur d'un objet Downloader

        param:
        url_server: string (url or ip)
        name_folder: string (stocké dans le %appdata%)
        custom_java: boolean
        logger: boolean default False
        sftp: boolean default False
        user: string default ''
        password: string default ''
        main_dir: string default ''
        max_recursion: integer default 1000

        return type: None
        """
        sys.setrecursionlimit(max_recursion)
        self.__recur = max_recursion
    ...
    def get_max_recursion(self): return self.__recur
    def set_max_recursion(self, new_value: int):
        self.__recur = new_value
        sys.setrecursionlimit(self.__recur)
    ...
    def get_recap(self):
        """
        Affiche un récapitulatif des informations stocké dans le Downloader
    ...
    def read_downloader(self, name="downloader.lxf") -> dict:
        """
        Permet de lire un fichier dit "downloader"

        param:
        name: string default 'downloader.lxf'
    ...
    def discord_presence(self, client_id: str, state='', details='', large_image='', large_text='', small_image='', small_text='') -> None:
        """
        Génére une instance Discord Rich Presence
    ...
    def generate_have_file(self):
        ...
        return generate_downloader(self.__folder_dir, self.__folder_dir, name="actual_file.lxf", max_recursion=self.__recur)
...
class Updater:
    def __init__(self, sftp_server: str, user: str, password: str, dir='', dir_server='', logger=False, max_recursion=1000):
        sys.setrecursionlimit(max_recursion)
        ...
        generate_downloader(self.__dir, self.__path, max_recursion=max_recursion)
        os.chdir(self.__path)
