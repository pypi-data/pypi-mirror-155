...
def der_2_activation_function(func: str, use_torch: bool = False):
    if not use_torch or not allow_torch:
        ...
        def logistic(x):
            ...
            fx = sigmoid(x)
            return zeros_3d.fill_diagonal_(fx * (1 - fx) * (1 - 2 * fx)) 
        ...
        def stanh(x):
            zeros_3d = zeros((x.size(0), x.size(0), x.size(0))).float()
            fx = tanh(x)
            return zeros_3d.fill_diagonal_(-2 * fx * (1 - fx ^ 2))
...
def der_3_activation_function(func: str, use_torch: bool = False):
    if not use_torch or not allow_torch:
        ...
        def logistic(x):
            ...
            fx = sigmoid(x)
            return zeros_4d.fill_diagonal_(fx * (1 - fx) * (1 - 2 * fx) * (1 - 3 * fx))
        ...
        def stanh(x):
            zeros_4d = zeros((x.size(0), x.size(0), x.size(0), x.size(0))).float()
            fx = tanh(x)
            return zeros_4d.fill_diagonal_(-2 * (1 - 4 * fx ** 2 + fx ** 4))
