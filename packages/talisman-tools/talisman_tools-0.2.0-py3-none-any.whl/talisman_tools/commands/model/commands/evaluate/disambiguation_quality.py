...
def _filter_with_predicate(doc: TalismanDocument, concepts_predicate: _Predicate):
    filtered_mentions = set()
    filtered_concepts = {}
    for ent in doc.content.entities:
        concepts = doc.content.concepts_for(ent)
        if not concepts_predicate(concepts):
            continue
        filtered_mentions.add(ent)
        if concepts is not None:
            filtered_concepts[ent] = concepts

    return filtered_mentions, filtered_concepts
...
def is_ambiguous(doc: TalismanDocument) -> bool:  # TODO: move it to helpers/tdm
    return any(isinstance(fact.value, tuple) for fact in doc.filter_facts(ConceptFact))
