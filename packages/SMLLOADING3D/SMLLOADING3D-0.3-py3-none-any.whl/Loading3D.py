...
class Bin:
    ...
    def draw_packed_bin(self):
        ...
        data.append(
            go.Mesh3d(
                x = [0, 0, int(self.bin_info[0]), int(self.bin_info[0]), 0, 0, int(self.bin_info[0]), int(self.bin_info[0])],
                y = [0, int(self.bin_info[1]), int(self.bin_info[1]), 0, 0, int(self.bin_info[1]), int(self.bin_info[1]), 0],
                z = [0, 0, 0, 0, int(self.bin_info[2]), int(self.bin_info[2]), int(self.bin_info[2]), int(self.bin_info[2])],
...
class Packer:
    ...
    def check_fit_batch_into_bin(self, bin, batch, keep_pack_result=False, keep_pack_if_fit_all=False):
        ...
        root_left_over_itemCbm = sum([it.get_cbm for it in batch])
        ...
        if left_over_itemCbm > 0 and root_left_over_itemCbm != 0:
            fit = False

        if not keep_pack_result or not (fit and keep_pack_if_fit_all):
            packing_problem.undo_pack()
            thrown_items = packing_problem.recycle_item_list[0] if len(packing_problem.recycle_item_list) > 0 else []
            for item in batch:
...
packer = Packer(list_of_bin, list_of_item)