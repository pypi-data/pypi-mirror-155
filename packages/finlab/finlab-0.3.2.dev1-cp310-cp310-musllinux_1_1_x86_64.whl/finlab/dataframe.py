...
class FinlabDataFrame(pd.DataFrame):
    ...
    def _index_str_to_date_season(self, detail=True):
      ...
      values = self.unstack()
      index = disclosure_dates.index.intersection(values.index)
      disclosure_dates = disclosure_dates.loc[index]

      df = pd.DataFrame({
        'value': values.loc[index].values,
        'disclosures': disclosure_dates.loc[index].values,
      }, index).reset_index()
