...
class StrategySunburst:
    """ Plot strategy sunburst

    Args:
      cost_weight(bool): if cost_weight is True,use cost value as weight.
                         if cost_weight is False,use market value as weight.
    """

    def __init__(self, cost_weight=True):
    ...
    def get_strategy_df(self, select_strategy=None):
        """process strategies dict data to dataframe

        Args:
          select_strategy(dict): select strategy name and set weight,default is all strategies and average weight,
                                 ex:{'低波動本益成長比':0.5,'研發魔人':0.3,, '現金':0.2}
        Returns:
            dataframe
        """
    ...
    def plot(self, select_strategy=None, path=None, color_continuous_scale='RdBu_r'):
        ...
        Args:
          ...
          path(lost):sunburst display root ,ex: ['s_name', 'market', 'category', 'stock_id'],
                                                ['market', 'category','stock_id','s_name']
          color_continuous_scale(str): plotly_colorscales,see detail on https://plotly.com/python/builtin-colorscales/
        Returns:
            figure
        """
