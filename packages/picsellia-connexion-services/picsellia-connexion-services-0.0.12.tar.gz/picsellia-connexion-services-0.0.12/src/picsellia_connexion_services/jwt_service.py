...
class JwtServiceConnexion(AbstractServiceConnexion):
    ...
    def get(self, path: str, data: dict = None, params: dict = None, stream=False):
        ...
        return requests.get(url=url, data=data, headers=self._build_headers(), params=params, stream=stream, cookies=self.cookies)

    @wrapped_request
    def post(self, path: str, data: dict = None, params: dict = None, files: Any = None):
        url = urljoin(self.host, path)
        return requests.post(url=url, data=data, headers=self._build_headers(), params=params, files=files, cookies=self.cookies)

    @wrapped_request
    def patch(self, path: str, data: dict = None, params: dict = None):
        url = urljoin(self.host, path)
        return requests.patch(url=url, data=data, headers=self._build_headers(), params=params, cookies=self.cookies)


    @wrapped_request
    def put(self, path: str, data: dict = None, params: dict = None):
        url = urljoin(self.host, path)
        return requests.put(url=url, data=data, headers=self._build_headers(), params=params, cookies=self.cookies)

    @wrapped_request
    def delete(self, path: str, data: dict = None, params: dict = None):
        url = urljoin(self.host, path)
        return requests.delete(url=url, data=data, headers=self._build_headers(), params=params, cookies=self.cookies)
