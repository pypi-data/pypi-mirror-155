...
class IOContainerData():
    ...
    def __init__(self):
        ...
        self.dir_input_enrichment_input='input/'
    ...
    def upload_frame_to_blob(self, container, file_name_blob, df_data, format):

        path = os.path.dirname(file_name_blob)

        if format.upper() == 'CSV':
            name_file = Path(file_name_blob).stem + ".csv"
            container_client = self.blob_service_client.get_blob_client(container, 
                                                                        blob=os.path.join(path, name_file))
                                                                        
            container_client.upload_blob(data=df_data.to_csv(index=False), 
                                        overwrite=True)
        elif format.upper() == 'PARQUET':
            name_file = Path(file_name_blob).stem + ".parquet"
            container_client = self.blob_service_client.get_blob_client(container,
                                                                        blob=os.path.join(path, name_file))

            container_client.upload_blob(data=df_data.to_parquet(index=False, engine='pyarrow'), 
                                        overwrite=True)
        else:
            raise Exception(f"Formato de arquivo invalido {format} use csv ou parquet")  


    def __is_dir(self, file_path):
        return '.' not in os.path.basename(file_path)

    def delete_to_blob(self, container_name, file_path):

        if( self.__is_dir(file_path) ):

            container_client = self.blob_service_client.get_container_client(container_name)
            blob_list = container_client.list_blobs(name_starts_with=file_path)

            for blob in blob_list:
                if( self.__is_dir(file_path) ):
                    self.delete_to_blob(container_name, blob.name)
                else:
                    remove_blob = self.blob_service_client.get_blob_client(container_name, file_path)
                    if(remove_blob.exists()):
    ...
    def write_dataframe_to_datalake(self, df, datalake_service_client, filesystem_name, dir_name, filename):
    ...
    def download(self, df, path_file, name_container,  sep=','):

        _path = f"abfs://{name_container}/{path_file}"

        logging.info(f"Arquivo processado de download {_path}")
        ...
        logging.info(f"Arquivo processado de download {_path} finalizado")
    ...
    def load(self, path_file, name_container,  sep=',', header=0, dtype=None, chunksize=None) -> pd.DataFrame:
        ...
        logging.info(f"Arquivo do load do arquivo {_path}")
...
if __name__ == '__main__':
    ...
    file_path = "datalake/filesystem/saleforce/bdm/input/Oportunidades Em andamento BDMs.xlsx_1.csv"

    path = os.path.dirname(file_path)
    name_file = Path(file_path).stem + ".parquet"
    name = os.path.join(path, name_file)
    ...
    ##transferContainer.delete_to_blob(transferContainer.name_container_silver,
    #"datalake/database/db_cateira/sfa_cateira_completa_diaria/")
