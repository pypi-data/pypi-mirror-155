...
class Bronze():
    ...
    def __break_file(self):
        ...
        logging.info(f"Nome do arquivo {file_name}")  
        logging.info(f"Caminho do arquivo {path_input_enrichment_input}")


        self.__delele_file(path_input_enrichment_input)
        ...
        if isinstance(df_data, pd.DataFrame):
            ...
            logging.info(f"Iniciando quebra do arquivo {file_input_enrichment_input} único")

            t = threading.Thread(target=self.ioContainerData.download, 
                            args=(df_data,
            ...
            logging.info(f"Fim da quebra do arquivo {file_input_enrichment_input} único")

        else:
        
            count = 1
            for _df_data in df_data:
                
                file_input_enrichment_input = os.path.join(path_input_enrichment_input, file_name + "_"+str(count)+".csv")

                logging.info(f"Iniciando quebra do arquivo {file_input_enrichment_input} do lote {count}")
                ...
                logging.info(f"Fim da quebra do arquivo {file_input_enrichment_input} do lote {count}")
                

        for thread in threads:
            thread.join()

        logging.info(f"Fim da quebra do arquivo {file_name}")

    def __delele_file(self, dir):
        
        self.ioContainerData.delete_to_blob(
            self.ioContainerData.name_container_silver,
            dir)
