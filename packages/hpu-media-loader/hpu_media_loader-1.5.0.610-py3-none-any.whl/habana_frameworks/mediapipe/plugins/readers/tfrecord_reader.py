...
class tfr_reader(media_ext_reader_op_impl):
    def __init__(self, params):
        ...
        if self.is_training and 'num_shards' in params and 'shard_index' in params:
            self.dataset_num_shards = params['num_shards']
            self.dataset_shard_index = params['shard_index']
        ...
        self.training_file_pattern = os.path.join(
            self.data_dir, 'train/train-*')
        self.val_file_pattern = os.path.join(
            self.data_dir, 'validation/validation-*')
        if(self.is_training):
            self._file_pattern = self.training_file_pattern
        else:
            self._file_pattern = self.val_file_pattern
        print("data_dir : ", self._file_pattern)
    ...
    def __iter__(self):
        ...
        if self.is_training and self.data_sharding:
            dataset = dataset.shard(self.dataset_num_shards,
                                    self.dataset_shard_index)
            dataset = dataset.shuffle(
                tf.cast(256 / self.dataset_num_shards, tf.int64))
        ...
        options.experimental_deterministic = not self.is_training
        ...
        if self.is_training:  # This part was copied from Coco reader
            ...
            if caching_mem_kib > mem_kib:
                ...
                dataset = dataset.cache().shuffle(_SHUFFLE_BUFFER)
