...
class Page(Base):
    ...
    def inPublic(self):
        '''Is p_self currently shown in the tool public page ?'''
        return self.req.rp == str(self.iid)
    ...
    pxSelector = Px('''
      <a for="page in pagesL" href=":page.getPublicUrl()">
       <span class=":'upage' if page.inPublic() else ''">:page.getShownValue()
       </span>
      </a>
      <select onchange="gotoURL(this)" if="pagesO">
       <option value="">:_('goto_link')</option>
       <option for="page in pagesO" value=":page.getPublicUrl()"
               selected=":page.inPublic()">:page.getShownValue()</option>
      </select>''',
