...
class Ref(Field):
    ...
    pxObjectActions = Px('''
     <div if="ifield.showActions"
      ...
      <div class="ibutton"
       ...
       <img class="clickable iconS" title=":text"
     ...
     <x if="mayAdd and not inPickList">
      <form class=":inMenu and 'addFormMenu' or 'addForm'"
       ...
       <x if="not createFromSearch">
        <img if="field.iconOut" src=":field.getAddIconUrl(url)"
        ...
        <input type=":field.getAddButtonType(create)"
               ...
               style=":field.getAddIconUrl(url, asBG=True) if addIcon else ''"
    ...
    def getAddIconUrl(self, url, asBG=False):
        '''Returns the URL to p_self's add icon, to be used as a background
           image or not (depending on p_asBG).'''
