...
class Devices:
    ...
    def __init__(self, _platform='mac'):
        self._platform = _platform

    def getDeviceIds(self):
        """获取所有连接成功的设备id"""
        Ids = list(os.popen(f'{Adb()._adb} devices').readlines())
        deviceIds = []
    ...
    def getDevicesName(self, deviceId):
        """获取对应设备Id的设备名称"""
        devices_name = os.popen(f'{Adb()._adb} -s {deviceId} shell getprop ro.product.model').readlines()
    ...
    def getPid(self, deviceId, pkgName):
        ...
        if platform.system() != 'Windows':
            result = os.popen(f"{Adb()._adb} -s {deviceId} shell ps | grep {pkgName}").readlines()
        else:
            result = os.popen(f"{Adb()._adb} -s {deviceId} shell ps | findstr {pkgName}").readlines()
    ...
    def getPkgname(self,devicesId):
        """获取手机所有包名"""
        pkginfo = os.popen(f"{Adb()._adb} -s {devicesId} shell pm list package")
        pkglist = []
        for p in pkginfo:
...
class Adb:
    ...
    def __init__(self):
        self._adbrootMac = os.path.dirname(os.path.abspath('.'))+'/adb/mac/adb'
        self._adbrootWin = os.path.dirname(os.path.abspath('.'))+'/adb/windows/adb'
        print(self._adbrootMac)
        self._adb = (self._adbrootMac,self._adbrootWin)[platform.system() == 'Windows']

    def shell(self, cmd ,deviceId):
        run_cmd = f'{self._adb} -s {deviceId} shell {cmd}'
