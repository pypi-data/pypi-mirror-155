...
def _should_run_seed(cycle: dict):
    ...
    logShouldRun(cycle, MODEL_ID, SEED_TERM_ID, should_run, key=MODEL_KEY)
...
def _run_input(cycle: dict, date: int):
    def run(input: dict):
        ...
        logShouldRun(cycle, MODEL_ID, term_id, True, key=MODEL_KEY)
...
def _should_run_input(input: dict):
    return all([
        ...
        _should_aggregate_input(input.get('term', {}))
    ])
