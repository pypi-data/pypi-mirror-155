...
class TokenTransformingAutoEncoderIterator(_TransformerIterator, _TokenIterator):
    ...
    def generate_learned_samples(self):
        ...
        for _ in range(self.iter_n):
            ...
            training_encoded_observed_arr = torch.from_numpy(
                training_encoded_observed_arr,
            )
            training_encoded_observed_arr = training_encoded_observed_arr.to(self.__ctx)
            training_encoded_observed_arr = self.pre_normalize(training_encoded_observed_arr)
            training_decoded_observed_arr = training_encoded_observed_arr.detach()
            training_target_arr = training_encoded_observed_arr.detach()

            test_encoded_observed_arr = torch.from_numpy(
                test_encoded_observed_arr,
            )
            test_encoded_observed_arr = test_encoded_observed_arr.to(self.__ctx)
            test_encoded_observed_arr = self.pre_normalize(test_encoded_observed_arr)
            test_decoded_observed_arr = test_encoded_observed_arr.detach()
            test_target_arr = test_encoded_observed_arr.detach()
            ...
            yield training_encoded_observed_arr.float(), training_decoded_observed_arr.float(), None, None, test_encoded_observed_arr.float(), test_decoded_observed_arr.float(), None, None, training_target_arr.float(), test_target_arr.float()
    ...
    def generate_samples_and_noises(self, test_mode=True):
        ...
        for key in range(len(self.seq_token_list) - self.seq_len):
            ...
            if batch == self.batch_size - 1:
                _decoded_observed_arr = torch.from_numpy(
                    decoded_observed_arr,
                )
                _decoded_observed_arr = _decoded_observed_arr.to(self.__ctx)
                encoded_observed_arr = np.random.uniform(
                    size=decoded_observed_arr.shape
                )
                _encoded_observed_arr = torch.from_numpy(encoded_observed_arr)
                _encoded_observed_arr = _encoded_observed_arr.to(self.__ctx)
                batch = 0
                yield _encoded_observed_arr.float(), _decoded_observed_arr.float(), None, None, _token_list
                _token_list = []
    ...
    def generate_inferenced_samples(self, media_token_list, test_mode=True):
        ...
        for key in range(len(self.seq_token_list) - self.seq_len):
            ...
            if batch == self.batch_size - 1:
                ...
                _decoded_observed_arr = _decoded_observed_arr.to(self.__ctx)
                _encoded_observed_arr = torch.from_numpy(
