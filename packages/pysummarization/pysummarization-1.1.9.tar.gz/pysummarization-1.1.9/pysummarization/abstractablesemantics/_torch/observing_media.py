...
from accelbrainbase.iteratabledata._torch.gauss_transformer_iterator import GaussTransformerIterator
...
from accelbrainbase.controllablemodel._torch.transforming_auto_encoder_controller import TransformingAutoEncoderController
from pysummarization.iteratabledata._torch.transformeriterator.token_transforming_auto_encoder_iterator import TokenTransformingAutoEncoderIterator
from pysummarization.iteratabledata._torch.transformeriterator.bi_sentence_transformer_iterator import BiSentenceTransformerIterator
...
class ObservingMedia(object):
    ...
    def __init__(
        ...
        computable_loss = torch.nn.CrossEntropyLoss()
        ...
        transformer_controller = TransformingAutoEncoderController(
            ...
            self_attention_activation_list=[torch.nn.GELU()],
            multi_head_attention_activation_list=[torch.nn.GELU()],
            fc_activation_list=[torch.nn.GELU()],
        ...
        self.compute_delta_f = compute_delta_f
    ...
    def extract_media(self, test_mode=True):
        ...
        for encoded_observed_arr, decoded_observed_arr, encoded_mask_arr, decoded_mask_arr, token_list in self.transformer_iterator.generate_samples_and_noises(
            ...
            if self.compute_delta_f is not None:
                delta_arr = self.compute_delta_f(
                    decoded_arr,
                    decoded_observed_arr
                )
            else:
                delta_arr = torch.sqrt(
                    torch.mean(
                        torch.square(
                            decoded_arr - decoded_observed_arr
                        ), 
                        dim=1, 
                        keepdim=True
                    )
            ...
            delta_arr = delta_arr.to('cpu').detach().numpy().copy()
            token_arr = np.array(token_list).reshape(-1, 1)
            data_arr = np.c_[delta_arr[:token_arr.shape[0]], token_arr]
...
def Main(
    ...
    media_n=5,
    form_n=25,
    mecab_system_dic="C:/Program Files/MeCab/dic/mecab-ipadic-neologd",
):
    ...
    tokenizable_doc.mecab_system_dic = mecab_system_dic
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--mecab_system_dic",
        type=str,
        default="C:/Program Files/MeCab/dic/mecab-ipadic-neologd",
        help="Dictionaly of MeCab."
    )
