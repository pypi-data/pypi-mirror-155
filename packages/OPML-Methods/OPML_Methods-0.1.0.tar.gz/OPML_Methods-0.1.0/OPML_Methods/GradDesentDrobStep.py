...
class GradDesentDrobStep:
    def find(self, z, l, x, iter, e, flag1, flag2, extr, pe, delta):
        """
        Функция находит экстремум методом градиентного спуска с дроблением шага.
        Parameters
        ===========
        :param z: str
        ...
        :param pe: float
            ввод значений параметра оценки?
        :param delta: float
            ввод значений параметра дробления?
        :return:
        ...
        while i < iter:
            ...
            sum_det = 0
            for j in range(len(calc_det)):
                sum_det += (calc_det[j][0]) ** 2

            if func.subs(podst_after) > (func.subs(podst) - pe * l * sum_det):
                l = l * delta
        ...
        if flag2 == 1:
            print(df)
            df.to_csv('GradDesentDrobStep_results.csv', index=False)
