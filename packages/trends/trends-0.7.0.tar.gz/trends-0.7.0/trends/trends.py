...
def _a_mean(trend_1: "Trend", trend_2: "Trend") -> float:
...
def _g_mean(trend_1: "Trend", trend_2: "Trend") -> float:
...
def _h_mean(trend_1: "Trend", trend_2: "Trend") -> float:
...
def config_mean():
    """Return configured mean function."""
    mean_fns = {"arithmetic": _a_mean, "geometric": _g_mean, "harmonic": _h_mean}
    mean_type = config("MEAN_TYPE", default="arithmetic")
    allowed_means = mean_fns.keys()
    if mean_type not in allowed_means:
        raise ValueError(f"mean_type must be in {allowed_means}")
    return mean_fns[mean_type]


def config_random():
    """Return configured random number generator."""
    rand_fns = {
        "normal": random.normalvariate,
        "lognormal": random.lognormvariate,
        "uniform": random.uniform,
    }
    rand_type = config("RAND_TYPE", default="uniform")
    allowed_rands = rand_fns.keys()
    if rand_type not in allowed_rands:
        raise ValueError(f"rand_type must be in {allowed_rands}")
    return rand_fns[rand_type]
