...
def submit(dialog):
    ...
    Mac, or if there are no absolute asset paths, we can submit the scene as-is. If we are on
    Windows, we need to make the paths relative. The contextmanager: relative_posix_filepaths()
    takes care of this and restores the project to it's original state afterwards.
    ...
    if asset_state in [k.ASSETS_OKAY, k.ASSETS_RELATIVE]:
        with save_render_file(filepath, cleanup=cleanup):
            handle_submissions(dialog)
        return

    with utils.relative_posix_filepaths():
        with save_render_file(filepath, cleanup=cleanup):
            handle_submissions(dialog)
