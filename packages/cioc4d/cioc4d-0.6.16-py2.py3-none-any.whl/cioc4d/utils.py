...
@contextmanager
def relative_posix_filepaths(dry_run=False):
    """
    ...
    active output paths. While the scene has posix paths, it is mpt guaranteed to be commpatible
    with Windows. It should be used to crerate a scene to ship to Linux only.

    NOTE: We can't simply strip drive letters to make absolute posix paths because the forward
    slashes for absolute paths are converted back to backslashes by c4d. 

    Assets can be anything in the asset_cache, i.e. anything returned by GetAllAssets(). If an asset
    has paramId = -1, it's possibly a redshift proxy, and needs to be handled differently.

    We also handle the paths stored in render data.
    ...
    with utils.relative_posix_filepaths(dry_run=True) as changes:
    ...
    for asset in assets:
        if asset["exists"]:
            if asset["paramId"] > -1:
                original_value = asset["owner"][asset["paramId"]]
            ...
            elif asset["owner"].GetType() == k.REDSHIFT_PROXY_TYPE:
    ...
    if not dry_run:
        for change in changes:
            ...
            c4d.WriteConsole(
                "Changed: '{}' to '{}'.\n".format(change["original_value"], change["new_value"])
            )
    ...
    try:
        ...
        if not dry_run:
            ...
            for change in changes:
                ...
                c4d.WriteConsole(
                    "Restored: '{}' to '{}'.\n".format(
                        change["new_value"], change["original_value"]
                    )
                )
            c4d.EventAdd()
...
def asset_state():
    ...
    ASSETS_OKAY: We are not on Windows (scene will work on Linux.)
    ASSETS_RELATIVE: We are on Windows but all paths are relative (fully portable scene.)
