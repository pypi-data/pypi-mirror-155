...
BASE_URL="http://censiuslogs-env.eba-m2wzz9xj.us-east-1.elasticbeanstalk.com/v1"
...
class CensiusClient(object):
    ...
    def revise_model(self, *args, **kwargs):
        ...
        TrainingData=kwargs["training_data"]
        IsRollingWindow=None
        reMapped=None
        MappedTo=None
        WindowSize=None
        WindowEndTime=None
        if "new" in TrainingData.keys():
            reMapped=True
            MappedTo=TrainingData["new"]
        else:
            reMapped=False
            MappedTo=None
        
        WindowStartTime=None
        if "production" in TrainingData.keys():
            WindowStartTime=TrainingData["production"]["start_time"]
            if "window_size" in TrainingData["production"].keys():
                WindowSize=TrainingData["production"]["window_size"]
                IsRollingWindow=True
            else:
                IsRollingWindow=False
                WindowEndTime=TrainingData["production"]["end_time"]
