...
class MaiAlquierDist_Symmetric:
    ...
    def sample_alpha(self, n_size=tf.TensorSpec(shape=1, dtype=tf.int64)):
        ...
        try:
            ...
            sampled = dist.sample(n_size)
        except:
            dist = np.random.dirichlet(alpha, size=n_size)
            sampled = tf.cast(dist, tf.float32)
         # [n_size, 2 ** self._qs]
        sampled = tf.expand_dims(sampled, axis=-1)  # [n_size, 2 ** self._qs, 1]
        sampled = tf.expand_dims(sampled, axis=-1)  # [n_size, 2 ** self._qs, 1, 1]
        return sampled
    ...
    def sample_dm(self, n_size, numpy_array=True):
        ...
        # self._qs, self._qs]
        alphas = self.sample_alpha(n_size)  # [n_size, 2 ** self._qs, 1, 1]
        alphas = self._cast_complex(alphas)
        ma_states_array = tf.multiply(alphas, haar_dm)  # [n_size, 2 ** self._qs, 2 ** self._qs, 2 ** self._qs]
        ma_states = tf.reduce_sum(ma_states_array,
                                  axis=1)  # [n_size, 2 ** self._qs --> traced out and dropped, 2 ** self._qs, 2 ** self._qs]
        # --> [n_size, 2 ** self._qs, 2 ** self._qs]
        if numpy_array:
