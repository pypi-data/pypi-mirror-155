...
class TokenClassificationBenchmark(AbstractBenchmark):
    ...
    def GetGoldStandard(self) -> dict:
        ...
        def _get_items_(items_collection):
            ...
            for items_in_sentence in items_collection:
                for item_in_sentence in items_in_sentence:
                    flatten_item.extend(items_in_sentence)
        ...
        for doc_name in self.PETdataset.GetDocumentNames():
            # golds[doc_name] = {ACTIVITY:                tc.GetDocumentActivities(doc_name),
            #                    ACTIVITY_DATA:           tc.GetDocumentActivityData(doc_name),
            #                    ACTOR:                   tc.GetDocumentActors(doc_name),
            #                    FURTHER_SPECIFICATION:   tc.GetDocumentFurtherSpecifications(doc_name),
            #                    XOR_GATEWAY:             tc.GetDocumentXORGateways(doc_name),
            #                    AND_GATEWAY:             tc.GetDocumentANDGateways(doc_name),
            #                    CONDITION_SPECIFICATION: tc.GetDocumentConditionSpecifications(doc_name)}

            golds[doc_name] = {ACTIVITY:                 _get_items_(self.PETdataset.GetDocumentActivities(doc_name)),
                               ACTIVITY_DATA:           _get_items_(self.PETdataset.GetDocumentActivityData(doc_name)),
                               ACTOR:                   _get_items_(self.PETdataset.GetDocumentActors(doc_name)),
                               FURTHER_SPECIFICATION:   _get_items_(self.PETdataset.GetDocumentFurtherSpecifications(doc_name)),
                               XOR_GATEWAY:             _get_items_(self.PETdataset.GetDocumentXORGateways(doc_name)),
                               AND_GATEWAY:             _get_items_(self.PETdataset.GetDocumentANDGateways(doc_name)),
                               CONDITION_SPECIFICATION: _get_items_(self.PETdataset.GetDocumentConditionSpecifications(doc_name))}
            ...
            for element_label in PROCESS_ELEMENT_LABELS:
                ...
                try:
                    if predictions[doc_name][element_label]:
                        print(doc_name, element_label, predictions[doc_name][element_label], len(predictions[doc_name][element_label]), type(predictions[doc_name][element_label]))
                        scores = self.ComputeClassEntityScores(self.goldstandard[doc_name][element_label],
                                                  predictions[doc_name][element_label])
    ...
    def __MicroPerClassStatistics(self, results: dict) -> dict:
        ...
        for rel_type in PROCESS_ELEMENT_LABELS:
            ...
            support = self.Support(tp=TP, fn=FN)
