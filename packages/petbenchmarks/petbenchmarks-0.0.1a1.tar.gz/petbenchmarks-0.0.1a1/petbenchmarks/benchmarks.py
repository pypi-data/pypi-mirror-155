...
class Benchmark:
    ...
    def BenchmarkApproach(self, results_filename='results'):
        """
            lunch the benchmark procedure
        """
        self.__define_type_of_prediction()

        # if self.benchmarking_level == SENTENCE_LEVEL:
        #     #  if it is token classification at the sentence level, only 1 predictions file is allowed
        #     if len(self.predictions) > 1:
        #         raise IndexError('Token Classification at the sentence level require exactly ONE predictions file, not more.')
        #
        #     #  Only token classification
...
class BenchmarkApproach(Benchmark):
    """
    does benchmarking in a line
    """
    def __init__(self,
                 tested_approach_name: str,
                 predictions_file_or_folder: str):

        super(BenchmarkApproach, self).__init__()
        # warnings.warn('TO BE TESTED')
        self.LoadPrediction(predictions_file_or_folder)
        resfile = str(Path(predictions_file_or_folder).parent.joinpath('results-'+tested_approach_name).absolute())
        self.BenchmarkApproach(resfile)
