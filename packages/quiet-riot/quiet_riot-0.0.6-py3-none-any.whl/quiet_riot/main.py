...
def main():
    ...
    # global_bucket = 's3://quiet_riot-global-bucket/'
    ...
    ecr_public_repo = f'quiet_riot-public-repo-{uuid.uuid4().hex}'
    ...
    ecr_private_repo = f'quiet_riot-private-repo-{uuid.uuid4().hex}'
    ecrprivate.create_repository(
        repositoryName=ecr_private_repo
    )
    # Create SNS Topic - Resource that has IAM policy attachment
    sns_topic = f'quiet_riot-sns-topic-{uuid.uuid4().hex}'
    sns.create_topic(
        Name=sns_topic
    )
    # Create s3 bucket to scan against for root account e-mail addresses.
    s3_bucket = f'quiet_riot-bucket-{uuid.uuid4().hex}'
    s3.create_bucket(
        Bucket=s3_bucket
    )
    ...
    # print(results_file)
    default_bucket_name = "quiet_riot-" + settings.account_no
        ...
        if prompt1 == 'yes':
            ...
            for i in range(0, len(buckets['Buckets'])):
                if len(buckets['Buckets']) != 0:
                    if 'quiet_riot-bucket' in buckets['Buckets'][i]['Name']:
            ...
            for i in range(0, len(public_repos['repositories'])):
                if len(public_repos['repositories']) != 0:
                    if 'quiet_riot-public-repo' in public_repos['repositories'][i]['repositoryName']:
                        print(
            ...
            for i in range(0, len(private_repos['repositories'])):
                if len(private_repos['repositories']) != 0:
                    if 'quiet_riot-private-repo' in private_repos['repositories'][i]['repositoryName']:
                        print(
            ...
            for i in range(0, len(sns_topics['Topics'])):
                if len(sns_topics['Topics']) != 0:
                    if 'quiet_riot-sns-topic' in sns_topics['Topics'][i]['TopicArn']:
            ...
            print('')
            # TODO: Create control flow logic to ask user if willing to upload valid principals to global quiet_riot bucket maintained by Righteous Gambit Research
            try:
