    ...
    def get_history_by_item_key(self, hosts=[], time_from='', time_till='', item_key='', data_type=3):
        ...
        match True:
            case True if type(hosts) != list or hosts == []:
                ...
                return None
            case True if type(time_from) != str or time_from == '':
                logger.error('ERROR!! time_from is not string or none')
                return None
            case True if type(time_till) != str or time_till == '':
                logger.error('ERROR!! time_till is not string or none')
                return None
