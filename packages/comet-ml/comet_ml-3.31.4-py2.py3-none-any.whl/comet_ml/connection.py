...
class RestServerConnection(object):
    ...
    def log_metrics_batch(self, metrics, compress=True):
        # type: (List[MessageBatchItem], bool) -> None
        endpoint_url = metrics_batch_url(self.server_address)

        payload = format_message_batch_items(
            batch_items=metrics, experiment_key=self.experiment_id
        )

        LOGGER.debug(
            "Sending metrics batch, length: %d, compression enabled: %s",
            len(metrics),
            compress,
        )
