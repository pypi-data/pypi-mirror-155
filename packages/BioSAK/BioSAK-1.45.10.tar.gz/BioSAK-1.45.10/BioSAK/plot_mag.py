...
def Plot_MAG(argument_list):
    ...
    output_txt_handle.write('Genome\tContig\tDepth\tGC\tLength(Kbp)\n')
    legend_elements = []
    num_list_depth = []
    num_list_gc = []
    ctg_len_list = []
    ...
    for each_mag in bin_file_list:
        ...
        pwd_each_mag_plot   = '%s/%s_plot_GC_vs_depth.pdf'   % (plot_folder, mag_no_ext)
        pwd_each_mag_txt    = '%s/%s_plot_GC_vs_depth.txt'   % (plot_folder, mag_no_ext)

        pwd_each_mag_txt_handle = open(pwd_each_mag_txt, 'w')
        pwd_each_mag_txt_handle.write('Contig\tDepth\tGC\tLength(Kbp)\n')
        legend_elements_sep = []
        num_list_depth_sep = []
        num_list_gc_sep = []
        ctg_len_list_sep = []
        ...
        for each_ctg in SeqIO.parse(pwd_each_mag, 'fasta'):
            ...
            pwd_each_mag_txt_handle.write('%s\t%s\t%s\t%s\n' % (each_ctg.id, ctg_depth, ctg_gc, ctg_len_kbp))
            output_txt_handle.write('%s\t%s\t%s\t%s\t%s\n'   % (mag_no_ext, each_ctg.id, ctg_depth, ctg_gc, ctg_len_kbp))
