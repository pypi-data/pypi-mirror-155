...
def just_plot(num_list_depth, num_list_gc, ctg_len_list, ctg_color_list, bin_file_list, legend_elements, output_plot):
...
def Plot_MAG(argument_list):
    ...
    plot_folder = '%s_GC_vs_depth_plots' % bin_folder
    if plot_sep is True:
    ...
    for each_mag in bin_file_list:
        ...
        for each_ctg in SeqIO.parse(pwd_each_mag, 'fasta'):
            ...
            ctg_len_list_sep.append(len(ctg_seq)/2000)
        ...
        legend_elements_sep.append(Patch(facecolor=color_list[index], edgecolor=color_list[index], label=each_mag))
        index += 1
        just_plot(num_list_depth_sep, num_list_gc_sep, ctg_len_list_sep, ctg_color_list_sep,
                  bin_file_list_sep, legend_elements_sep, pwd_each_mag_plot)
