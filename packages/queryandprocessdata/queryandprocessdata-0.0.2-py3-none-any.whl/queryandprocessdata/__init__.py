    ...
    def getConnection(self):
        ...
        if self.dbDetails != None:
            if self.dbDetails['DATABASETYPE'] == '1':
                ...
                if self.isPool == 'true':
                    pool = db.SessionPool(user=userName, password=password, dsn=url, min=self.minPool, max=self.maxPool,
                            ...
                            if detail['calc_type'] == 'S':
                                ...
                                if detail['calc_expression'] != None:
                                    self.df[detail['col_name'].lower().strip()] = self.df.apply(
                                        lambda row: self.getSqlResult(row, pool, detail), axis=1)
                                else:
                                    self.df[detail['col_name'].lower().strip()] = self.df.parallel_apply(lambda x: None,
                                                                                                         axis=1)
                                ...
                                if detail['calc_expression'] != None:
                                    self.df[detail['col_name'].lower().strip()] = self.df.apply(
                                        lambda row: self.getLookUpValue(row, pool), axis=1)
                                else:
                                    self.df[detail['col_name'].lower().strip()] = self.df.apply(
                                        lambda row: self.getLookUpValue(row, pool), axis=1)
                                ...
                                if detail['calc_expression'] != None:
                                    ...
                                    if len(columnArrList) == 1:
                                        ...
                                        del columnArrList[0]
                                        self.df[detail['col_name'].lower().strip()] = self.df.groupby(columnArrList)[
                                            cumsumColumn].cumsum()
                                else:
                                    self.df[detail['col_name'].lower().strip()] = self.df.parallel_apply(lambda x: None,
                                                                                                         axis=1)
