...
def getLatestTransactionRewardBlock(connection, schema, token:str, address:str, chain=None, chain_id=None):
    conditions = ""
    params = []
    if chain is None and chain_id is None:
        return []
    ...
    query = sql.SQL("SELECT 'tax_transaction' AS table, MAX(block_number) as latest_block_number FROM {0}.tax_transaction \
                    WHERE 1=1 AND public_address = %%s AND token = %%s %s \
                    UNION ALL \
                    SELECT 'tax_reward' AS table, MAX(latest_block_number) as latest_block_number FROM {0}.tax_reward \
                    WHERE 1=1 AND public_address = %%s AND token = %%s %s" % (conditions,conditions)).format(sql.Identifier(schema))

    result = herokuDbAccess.fetchDataInDatabase(query, [address,token,*params,address,token,*params], connection)  
    return result
