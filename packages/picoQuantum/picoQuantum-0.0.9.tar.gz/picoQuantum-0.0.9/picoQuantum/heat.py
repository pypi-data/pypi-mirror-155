...
class ABCDModel():
    ...
    def qubit(self, omega, element):
        Lq = (element['Lq']/np.abs(np.cos(self.flux))*(np.sqrt(1+(0.1**2)*(np.tan(self.flux))**2)))
    ...
    def calculate_S21(self):
        ...
        self.S21 = np.empty((len(self.fluxes), len(self.omegas)), dtype = np.float64)
    ...
    def heat_flux(self,omegas,fluxes):
        
        self.compute_ABCD(omegas, fluxes)
        self.calculate_S21()
