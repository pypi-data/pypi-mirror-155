import argparse
import logging
from typing import Iterable, Tuple, List, NamedTuple, Callable

from colorama import Fore, Style

#pylint: disable = import-error
import exceptions
from parsing import requesters, parsers
...
def parse_list_pages(urls: Iterable[str], file_path: str = None) -> str:
    ...
    writer: writers.TableWriter = writers.get_writer_by_extension(file_format)(rows)
    if writer is None:
...
def handle_exceptions(func: Callable, *args, **kwargs) -> None:
...
def main() -> None:
    ...
    handle_exceptions(lambda path: logging.info("Saved to -> %s", path),
                                get_method_by_mode(args.mode)(args.file_path))
