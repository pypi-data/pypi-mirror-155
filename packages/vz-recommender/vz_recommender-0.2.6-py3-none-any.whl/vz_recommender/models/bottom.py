...
class ContextHead(nn.Module):
    ...
    def __init__(self, deep_dims, item_embedding, deep_embed_dims=100, num_wide=0, 
                 num_shared=1, shared_embeddings_weight=None, tokenizer=None, nlp_encoder=None):
        super().__init__()
        ...
        self.nlp_encoder = nlp_encoder.to(self.device)

    def forward(self, deep_in:List[Tensor], wide_in:List[Tensor]=None, shared_in:List[Tensor]=None, search_in:Tensor=None):
        """
        Args:
        ...
        """
        encoded_search = self.tokenizer(list(search_in), return_tensors='pt', truncation=True, padding=True).to(self.device)
        search_embedding = self.nlp_encoder(**encoded_search).last_hidden_state[:,0,:]
        ...
        if wide_in is not None:
            ...
            ctx_out = deep_out
        ctx_out = torch.cat((ctx_out, search_embedding), dim=1)
...
class BSTBottom(nn.Module):
    ...
    def __init__(self, deep_dims, seq_dim, seq_embed_dim, deep_embed_dims, seq_hidden_size, tokenizer, nlp_encoder,
        ...
        self.dense1 = torch.nn.Linear(in_features=768+len(deep_dims)*deep_embed_dims+num_wide+seq_embed_dim+seq_embed_dim+(num_shared*seq_embed_dim), out_features=2 * seq_embed_dim)
