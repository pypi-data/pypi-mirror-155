...
class ContextHead(nn.Module):
    ...
    """
    def __init__(self, deep_dims, item_embedding, deep_embed_dims=100, num_wide=0, nlp_dim=0, nlp_embed_dim=768,
        ...
        self.search_dense = nn.Linear(in_features=nlp_dim, out_features=nlp_embed_dim)
    ...
    def forward(self, deep_in:List[Tensor], wide_in:List[Tensor]=None, shared_in:List[Tensor]=None, search_in=None):
        """
        Args:
        ...
        Return: 
            ctx_out: Tensor, shape [batch_size, len(deep_dims)*deep_embed_dims+(num_shared*seq_embed_dim)+num_wide+nlp_dim]
        """
        ...
        if search_in is not None:
            search_in = self.search_dense(search_in)
        ctx_out = torch.cat((ctx_out, search_in), dim=1)
...
class BSTBottom(nn.Module):
    ...
    def __init__(self, deep_dims, seq_dim, seq_embed_dim, deep_embed_dims, seq_hidden_size,
        ...
        self.dense1 = torch.nn.Linear(in_features=nlp_embed_dim+len(deep_dims)*deep_embed_dims+num_wide+seq_embed_dim+seq_embed_dim+(num_shared*seq_embed_dim), out_features=2 * seq_embed_dim)
