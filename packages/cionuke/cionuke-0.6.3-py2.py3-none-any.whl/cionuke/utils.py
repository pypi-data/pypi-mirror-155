...
def create_directories_on(write_nodes=None):
    """
    Turn on create_directories on write nodes for the submission.
    """
    
    write_nodes = write_nodes or nuke.allNodes("Write")
...
@contextlib.contextmanager
def transient_save(cio_filename):
    """
...
def resolve_submission(submitter, **kwargs):
    ...
    for component in components.COMPONENT_RESOLVE_ORDER:
        submission.update(getattr(components, component).resolve(submitter, **kwargs))
    return submission        
