...
class LearningSwitchController(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Setup logging
        logging.basicConfig(filename=f"/csle_sdn_controller_{self.controller_type}.log", level=logging.INFO)
    ...
    def _packet_in_handler(self, ev) -> None:
        ...
        logging.info(f"[SDN-Controller {self.controller_type}] received packet in, DPID:{datapath_id}, "
    ...
    def _port_status_handler(self, ev) -> None:
        ...
        if reason == openflow_protocol.OFPPR_ADD:
            logging.info(f"[SDN-Controller {self.controller_type}] a port {port_no} was added to switch with "
                         f"DPID: {datapath_id}")
        elif reason == openflow_protocol.OFPPR_DELETE:
            logging.info(f"[SDN-Controller {self.controller_type}] port {port_no} was removed from switch with "
                         f"DPID: {datapath_id}")
        elif reason == openflow_protocol.OFPPR_MODIFY:
            logging.info(f"[SDN-Controller {self.controller_type}] port {port_no} on switch with "
                         f"DPID: {datapath_id} was modified")
        else:
            logging.info(f"[SDN-Controller {self.controller_type}] Illegal port state: {port_no}, {reason}")