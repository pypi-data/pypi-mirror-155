...
class TestPairwiseStatistics(object):
    ...
    def _assert_statistic_dataframes(self, actual):
        ...
        for label_configuration, expected_value in expected.items():
            pd.testing.assert_frame_equal(actual[label_configuration], expected_value)
    ...
    def test_pairwise_probabilities_as_dataframe_not_a_valid_label_configuration(
        ...
        expected_error_msg = (
            ...
            "\(0, 0\), \(0, 1\), \(1, 0\) and \(1, 1\)"  # noqa: W605 pylint: disable=anomalous-backslash-in-string
        )
