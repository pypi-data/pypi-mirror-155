...
def test_score_to_file(
    ...
    with open(output_file, "rb") as result_file:
        assert result_file.read() == batch_prediction_job_data_csv
...
def test_score_pandas(
    ...
    _, df = BatchPredictionJob.score_pandas("5ce1138c962d7415e076d8c6", df)
...
def test_score_using_multipart(
    ...
    for _ in range(chunk_size_mb * number_of_parts):
...
def test_multipart_async_false_reupload(
    ...
    chunk_size_mb = 5
    number_of_parts = 2
    for _ in range(chunk_size_mb * number_of_parts):  # chunks are 5 mb
        csvdata += "{},{}\n".format("f" * 1024 * 1024, "b")
