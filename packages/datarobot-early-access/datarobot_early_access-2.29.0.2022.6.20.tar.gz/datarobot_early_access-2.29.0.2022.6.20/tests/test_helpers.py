...
class HelpersTestCase(unittest.TestCase):
    def test_cant_create_without_cv(self):
        with self.assertRaises(TypeError):
            BasePartitioningMethod(validation_type="CV")  # pylint: disable=no-value-for-parameter

        with self.assertRaises(TypeError):
            BasePartitioningMethod(cv_method="user")  # pylint: disable=no-value-for-parameter
