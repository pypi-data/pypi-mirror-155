from cvpr_2022_itt_pkg.core.core import *
from cvpr_2022_itt_pkg.ece.ece import *
import numpy as np
import math
...
# Optics Functions
def fraunhofer(wave, T, D=np.inf, wvl=500e-9, z=10000, mask='circle', pad=0):
    """
    ...
    :param D: float
        Diameter of the aperture. By default set to D=np.inf for plane wave (though, you will get aliasing)
    :param wvl: float
...
def fresnel(wave, T, D=np.inf, wvl=500e-9, z=10000, mask='circle'):
    """
...
def phase2psf(phase, T, D, f, wvl=500e-9, L=-1, mask='circle'):
    ...
    with the option of the distance L being provided to the function to know the PSF size in terms of the dimensions
...
def wave2psf(wave, T, D, f, wvl=500e-9, L=-1, mask='circle'):
    ...
    the distance L being provided to the function to know the PSF size in terms of the dimensions of the source object.
...
def otf_calc(pup_fun, T, D, f, wvl):
    ...
    w = D/2
    f_cutoff = w / (wvl * f)
    a, b, P = fft2(pup_fun, T)
    xx, yy, otf = np.abs(ifft2(P * P, mesh_spread_2d(a)))
    #xx, yy = meshgrid(linspace(-1/2, 1/2, pup_fun.shape[0]))
    #scaling = 2*f_cutoff
    return xx, yy, otf/np.amax(otf)
...
def defocus_abgen(norm_grid, L_true, L_selected, wvl=500e-9):
    '''
    A function that returns a defocus (difference of spheres) aberration in radians.
    ...
    for i in range(Z):
        for j in range(Z):
            ...
            if (abs(mi) == abs(mj)) and (np.mod(i - j, 2) == 0):
                ...
                coef1 = 0.0072 * (np.pi ** (8.0/3.0)) * np.sqrt((ni + 1) * (nj + 1)) * \
...
def zernikeGen(N, coeff, axis=-1):
    ...
    # Setting up 2D grid
    x_grid, y_grid = meshgrid(np.linspace(-1, 1, N))
...
def radialZernike(x_grid, y_grid, z_ind):
    ...
    for k in range(int((n - m)/2 + 1)):
        temp = (-1) ** k * math.factorial(n - k) / (math.factorial(k) * math.factorial((n + m)/2 - k)
