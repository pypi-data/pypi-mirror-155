    ...
    def job_callback(docs: List[firestore.DocumentSnapshot],
        ...
        for document, change in zip(docs, changes):
            if change.type in {ChangeType.MODIFIED}:
                ...
                status = data['status']
                if status == 'MSA_QUEUE':
                    bar.set_postfix_str('MSA Queue  ')
                    bar.update()
                elif status == 'MSA_RUNNING':
                    bar.set_postfix_str('MSA Running')
                    bar.update()
                    ...
                    bar.set_postfix_str('MSA Complete')
                    bar.update(48)
                elif status == 'FOLD_QUEUE':
                    bar.set_postfix_str('Fold Queue ')
                    bar.update()
                elif status == 'FOLDING':
                    bar.set_postfix_str('Folding    ')
                    bar.update()
                elif status == 'DONE' or status == 'MSA_FAILED' or status == 'FOLDING_FAILED':
                    bar.set_postfix_str(status.ljust(12))
                    bar.n = 100
                    bar.update(0)
                    bar.close()
    ...
    def seq_callback(docs: List[firestore.DocumentSnapshot],
        ...
        for document, change in zip(docs, changes):
            if change.type in {ChangeType.MODIFIED}:
                ...
                model = progress // 2 + 1
                bar.set_postfix_str(f'Sequence {document.id}, model {model}/5, {action}')
