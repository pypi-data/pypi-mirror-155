...
public:
  explicit HorovodAllreduceOp(OpKernelConstruction* context)
    ...
    if (horovod::common::ShouldUseOrderedHccl()) {
      OP_REQUIRES_OK(
          context, context->GetAttr("_group_order_ids", &group_order_indices_));
      OP_REQUIRES_OK(context, context->GetAttr("_order_id", &order_index_));
    }
  }
...
public:
  ...
  void ComputeAsync(OpKernelContext* context, DoneCallback done) override {
    ...
    common::ReadyEventList ready_event_list;
    #if HAVE_GPU
    auto ready_event_list.AddReadyEvent(std::shared_ptr<common::ReadyEvent>(RecordReadyEvent(context)));
    #endif
    auto hvd_context = std::make_shared<TFOpContext>(context);
    auto hvd_tensor = std::make_shared<TFTensor>(tensor);
...
REGISTER_KERNEL_BUILDER(Name("HorovodReduceScatter")
                            .Device(DEVICE_HPU),
...
REGISTER_OP("HorovodReduceScatter")
    ...
    .Doc(R"doc(
Perform an MPI ReduceScatter on a tensor.
