...
Status ReduceScatterOp::PrepareOutputAndParams(TensorTableEntry& e,
                                   int64_t& num_elements_reduce_scatter,
                                   int64_t& num_elements_remainder) {
  auto& process_set = global_state_->process_set_table.Get(e.process_set_id);
  int global_size = process_set.controller->GetSize();
  ...
  int64_t slice_first_dim {tensor_first_dim / global_size};
  int64_t remainder {tensor_first_dim - (slice_first_dim * global_size)};

  bool last_rank {
    (process_set.controller->GetSize()-1) == process_set.controller->GetRank()};

  num_elements_reduce_scatter = slice_first_dim * single_slice_shape.num_elements();
  num_elements_remainder = remainder * single_slice_shape.num_elements();
  // Allgather output will have shape of:
  // (sum of first dimension of every tensor) x (tensor slice shape).
  TensorShape output_shape;
  output_shape.AddDim(last_rank ? slice_first_dim + remainder : slice_first_dim);
  output_shape.AppendShape(single_slice_shape);
