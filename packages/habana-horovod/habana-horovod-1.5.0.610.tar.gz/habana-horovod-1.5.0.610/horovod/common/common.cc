...
bool ShouldUseOrderedHccl() {
  static const bool tf_signaling_from_encap_op_enabled{
      CheckBoolEnvVariable("TF_USE_SIGNALING_FROM_ENCAP_OP", false)};
  return tf_signaling_from_encap_op_enabled;
}
...
int HcclSignalingFromEncapOpMinBatchSize () {
  static const int max_batch_size{
      ReadIntEnvVariable("HOROVOD_HCCL_SIGNALING_FROM_ENCAP_OP_MIN_BATCH_SIZE_KiB", 0)};
