...
def test_netconf_upstream_ignore_hello():
    from mb_netmgmt.netconf import Handler
    ...
    with mb(
        ...
        handler.open_upstream()
        handler.upstream_channel.send(
            f'<rpc xmlns="urn:ietf:params:xml:ns:netconf:base:1.0" message-id="{message_id}"><get-config>running</get-config></rpc>'
            + MSG_DELIM.decode()
        )
        proxy_response = handler.read_proxy_response()
        assert proxy_response == create_proxy_response(message_id)
