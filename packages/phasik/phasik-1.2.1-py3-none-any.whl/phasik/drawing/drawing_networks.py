...
def standard_params(color):
    ...
    return {
        ...
        "node_size": 100,
        "bbox": dict(
            facecolor=color, edgecolor="black", boxstyle="round, pad=0.2", alpha=1
        ),
    }
...
def draw_graph(
    ...
    _draw_graph(graph, pos, ax, label_nodes, color, edge_widths, edge_colors, edge_cmap, edge_vmin, edge_vmax)


def _draw_graph(graph, pos, ax, label_nodes, color, edge_widths, edge_colors, edge_cmap=None, edge_vmin=None, edge_vmax=None):
    ...
    im = nx.draw_networkx_edges(graph, ax=ax, pos=pos, edge_cmap=edge_cmap, edge_vmin=edge_vmin, edge_vmax=edge_vmin, **params_edges)
    if label_nodes:
...
def highlight_subgraphs(graphs, colors, ax=None, pos=None, label_nodes=True):
    ...
    if pos is None:
        pos = graphviz_layout(graphs[0], prog="neato")

    for graph, color in zip(graphs, colors):
        _draw_graph(graph, pos, ax, label_nodes, color)
