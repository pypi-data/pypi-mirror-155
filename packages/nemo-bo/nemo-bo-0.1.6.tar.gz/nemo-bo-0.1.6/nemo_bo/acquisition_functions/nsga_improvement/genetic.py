...
class GeneticAlgorithm(Algorithm):
    ...
    def _initialize_infill(self):
        ...
        if self.problem.variables.num_cat_var > 0:
            for individual_idx, individual in enumerate(pop):
                ...
                ) + np.array(self.problem.variables.lower_bounds)
                X_temp = self.problem.variables.categorical_values_euc(X_temp.reshape(1, -1))
                pop[individual_idx].X = (X_temp.flatten() - np.array(self.problem.variables.lower_bounds)) / (
                    np.array(self.problem.variables.upper_bounds) - np.array(self.problem.variables.lower_bounds)
                )
    ...
    def _infill(self):
        ...
        if self.problem.variables.num_cat_var > 0:
            for individual_idx, individual in enumerate(off):
                X_temp = (
                    individual.X
                    * (np.array(self.problem.variables.upper_bounds) - np.array(self.problem.variables.lower_bounds))
                ) + np.array(self.problem.variables.lower_bounds)
                X_temp = self.problem.variables.categorical_values_euc(X_temp.reshape(1, -1))
                off[individual_idx].X = (X_temp.flatten() - np.array(self.problem.variables.lower_bounds)) / (
                    np.array(self.problem.variables.upper_bounds) - np.array(self.problem.variables.lower_bounds)
                )
