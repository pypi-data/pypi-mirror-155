...
def create_predictor_list(predictor_type: List[str | Base_Model]) -> List[Base_Model]:
    if predictor_type is not None:
        if isinstance(predictor_type, list):
            ...
            for predictor in predictor_type:
                if isinstance(predictor, str):
                    if predictor == "gp":
                        ...
                        raise ValueError(
                            f'Predictor type "{predictor}" is not natively supported. Please provide the class of the external predictor that inherits from the BasePredictor class'
                        )
                else:
                    if isinstance(predictor, Base_Model):
                        predictor_type_list.append(predictor)
                    else:
                        raise ValueError(f'Predictor type "{predictor}" does not inherit from the BasePredictor class')
            ...
            if predictor_type == "gp":
                ...
                raise ValueError(
                    f'Predictor type "{predictor_type}" is not natively supported. Please provide the class of the external predictor that inherits from the BasePredictor class'
                )
        else:
