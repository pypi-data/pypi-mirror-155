# -*- coding: utf-8 -*-
"""
Created on Tue May 31 10:08:29 2022
...
class LSTM_hdw(Activations):
    ...
    def __init__(
        ...
        try:
            self.W_in  = np.loadtxt(W_in, dtype='str')
            self.W_rec = np.loadtxt(W_rec, dtype='str')
    ...
    def forward(self, X):
        ...
        for i in range (t):
            if i == 0:
                h_ini = np.full(self.output_len, initial[self.Quant[0]][self.Quant[1]])
                c_ini = np.full(self.output_len, initial[self.Quant[0]][self.Quant[1]])
    ...
    def SFE(self, c_in, h_in, X_in):
        ...
        self.c_out = self.toQuant(c_out)
        self.T_c = T_c
        self.h_out = self.toQuant(h_out)
    ...
    def VMM(self, X, W_in, h_in, W_rec, Bias):
        ...
        p2 = out.copy()
        dtype = self.Quant[0]
        for i in range(dim_rec):
            for j in range(dim_in):
                p1[i] += int(W_in[i, j], dtype) * int(X[j], dtype) >> self.fract_width
            for k in range(dim_rec):
                p2[i] += int(W_rec[i, k], dtype) * int(h_in[k], dtype) >> self.fract_width
            out[i] = p1[i] + p2[i] + int(Bias[i], dtype)
        return out
    ...
    def Hadamard(self, arr1, arr2):
        ...
        dtype = self.Quant[0]
        for i in range (dim):
            out[i] = int(arr1[i], dtype) * int(arr2[i], dtype) >> self.fract_width
        return out
