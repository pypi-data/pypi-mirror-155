...
def DownloadLink():
    ...
    location = input(f"Provide the location to download the video [{download_path}] : ")  # get the user input
    if 'pornhub' in user_input:
        print("PHUB")
        # phubDownloader.downloader(user_input)
        NewphubDownloader.downloader(user_input, location)
    elif 'iplt20' in user_input:
        print("IPL")
        # IPLNewDownloader.downloader(user_input)
