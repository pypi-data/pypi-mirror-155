...
def show_dmsp(sat_id=None, pole='N', band='LBHS', dt_c=None, file_dir=None):
    file_name = 'DMSP_' + dt_c.strftime(
        '%Y-%m-%d_%H%M') + '_' + sat_id.upper() + '_SSUSI_' + band + '_SSJ_SSM_SSIES_' + pole + '.png'
    file_path_out: pathlib.Path = file_dir / file_name

    # if file_path_out.is_file():
    #     return
    ...
    try:
        ...
        pcolormesh_config['c_lim'] = [100, 2000]
        ...
        orbit = OrbitPosition_SSCWS(dt_fr=dt_c - datetime.timedelta(minutes=20),
                                    dt_to=dt_c + datetime.timedelta(minutes=20), sat_id='dmsp' + sat_id.lower())
        sc_dt = orbit['SC_DATETIME'].value.flatten()
        sc_lat = orbit['SC_GEO_LAT'].value.flatten()
        sc_lon = orbit['SC_GEO_LON'].value.flatten()
        sc_alt = orbit['SC_GEO_ALT'].value.flatten()
        sc_coords = {'lat': sc_lat, 'lon': sc_lon, 'height': sc_alt}
        panel1.overlay_sc_trajectory(sc_ut=sc_dt, sc_coords=sc_coords, cs='GEO', time_minor_tick=None)

        # Overlay sites
        # panel1.overlay_sites(site_ids=['TRO', 'ESR'],
        #                      coords={'lat': [69.58, 78.15], 'lon': [19.23, 16.02], 'height': [0, 0]},
        ...
        print("Error in SSUSI data! Check the data availability first!")
    ...
    try:
        ...
        print("Error in SSJ and SSM data!")
...
if __name__ == "__main__":
    ...
    sat_id = 'f19'
    dt = datetime.datetime.strptime('20151117 131200', "%Y%m%d %H%M%S")
