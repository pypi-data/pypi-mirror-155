...
class Job:
    ...
    def _get_progress(self, job: JobMetadata) -> Optional[str]:
        ...
        # TODO: this is only for stress tests, enable for CT runs.
        test_batches = job.job_data.stress.progress.test_run.test_batches
    ...
    def get_status(
        ...
        with self._backend.get_job_reader_stub() as job_reader, self._backend.get_model_testing_stub() as model_tester:  # pylint: disable=line-too-long
            job_req = GetJobRequest(
                job_id=self._job_id, job_type=self._job_type, view=JobView.JOB_VIEW_FULL
            )
            try:
                job: JobMetadata = job_reader.GetJob(job_req).job
            except grpc.RpcError as e:
