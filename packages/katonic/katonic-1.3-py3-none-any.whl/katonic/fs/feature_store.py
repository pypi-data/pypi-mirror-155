...
from katonic.fs.core.provider import Provider, get_provider
from katonic.fs.entities.entity import Entity
from katonic.fs.entities.feature_view import FeatureView
from katonic.fs.errors import EntityNotFoundException, FeatureViewNotFoundException
from katonic.fs.registry import Registry
from katonic.fs.repo_config import RepoConfig, load_repo_config
...
class FeatureStore:
    ...
    def get_registry_info(self, project_name: Optional[str] = '', user_name: Optional[str] = '') -> pd.DataFrame:
        """
        ...
        if project_name or user_name:
            ...
            raise TypeError("get_registry_info() missing 1 required positional argument: 'project_name' or 'user_name'")
    ...
    def write_table(
        ...
        self.views_to_update = list(filter(lambda ob: isinstance(ob, FeatureView), objects))

        _validate_feature_views(self.views_to_update)

        if len(self.views_to_update) + len(entities_to_update) != len(objects):
    ...
    def publish_table(
        ...
        feature_views_to_materialize = self.views_to_update
    ...
    def get_online_features(
        ...
        grouped_refs = _group_feature_refs(
            _feature_refs, all_feature_views
        )

        provider = self._get_provider()
        entity_name_to_join_key_map = {
            entities: entities for view in all_feature_views for entities in view.entities
        }
    ...
    def _populate_result_rows_from_feature_view(
        ...
        table: FeatureView,
    ) -> List[Tuple[Optional[datetime], Optional[Dict[str, Any]], Optional[Dict[str, Any]]]]:
        result_rows = []
        ...
        for read_row, entity_key in zip(read_rows, entity_keys):
            ...
            if feature_data is None:
                raise ValueError("No data found related to given features and entities id.")
...
def _validate_feature_views(feature_views: List[FeatureView]):
    ...
    for fv in feature_views:
        ...
        if case_insensitive_fv_name in fv_names:
            raise ValueError(
                f"More than one feature view with name {case_insensitive_fv_name} found. Please ensure that all feature view names are case-insensitively unique."
            )
        else:
...
def _feature_id(feature_views: List[str], all_features: List[str]):
    ...
    if len(feature_views) == 1:
        ...
        return list(
            map(lambda feature: f'{feature_view_name}:{feature}', all_features)
        )