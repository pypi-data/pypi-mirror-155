...
class read_igc:
	...
	def calculate_parameters(self):
		...
		self.infos.tracklog_distance = self.track.get_tracklog_length()
		self.infos.takeoff = self.track.get_takeoff()
		self.infos.landing = self.track.get_landing()
...
class Track:
	...
	def get_takeoff(self):
		return Position(self.latitude[0], self.longitude[0])

	def get_landing(self):
		return Position(self.latitude[-1], self.longitude[-1])
...
class FlightInfos:
	def __init__(self):
		...
		self.duration = None
		self.tracklog_distance = None
		self.takeoff: Position = None
		self.landing: Position = None
