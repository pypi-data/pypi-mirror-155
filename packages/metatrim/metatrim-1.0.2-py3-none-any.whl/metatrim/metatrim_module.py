    ...
    def validate_primers(self):
        ...
        for base in self.seq:
            if base.upper() not in iupac_amb:
                ...
                err = True
        if err == True:
            logger.error(f'Primer {seq} has invalid base(s) {", ".join(invalid)}. '
                'Aborting.')
            print(f'Primer {seq} has invalid base(s) {", ".join(invalid)}. '
                'Aborting.')
            exit()
...
# main function
def metatrim_driver(indices, deg, forward, reverse, length, min_length, fastq_tups,
    out_dir):
    # write header of summary file
    columns = ['Sample', 'total_reads', 'r1_index', 'i_in_r1', 'p1_in_r1', 
               'p2_in_r1', 'dimers_r1', 'r2_index', 'i_in_r2', 'p1_in_r2', 
               'p2_in_r2', 'dimers_r2', 'metatrimmed']
    with open(os.path.join(out_dir, f'metatrim_summary_{dt_string}.txt'),
        'wt') as summary_file:
        header = "\t".join(i for i in columns)
    ...
    for tup in fastq_tups:
        ...
        summary = {'Sample': sample, 'total_reads': 0,
        ...
        if indices[0] is not None:
            index_r1 = index_count(tup[0], indices[0], 1000, deg)
            summary['r1_index'] = index_r1[0]
        if indices[1] is not None:
            index_r2 = index_count(tup[1], indices[1], 1000, deg)
        ...
        with gzip.open(tup[0], 'rt') as infile_r1:
            with gzip.open(tup[1], 'rt') as infile_r2:
