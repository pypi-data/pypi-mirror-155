...
def get_insert_update_semaphore_command(connection_type: str):
	if connection_type.lower() == 'mysql':
		return 'insert into openk_semaforo.semaforo(identificador, identificador2, tipo_id, data_alteracao, data_sincronizacao) values (%s, %s, %s, %s, null) on duplicate key update data_alteracao = now()'
	elif connection_type.lower() == 'oracle':
		return '''
