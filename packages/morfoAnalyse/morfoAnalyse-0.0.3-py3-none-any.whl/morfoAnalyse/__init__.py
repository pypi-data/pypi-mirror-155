def morfoAnaliz(inputW, root, OT, FEL):
        ...
        if len(otstem) > 0:
            root_3.append('Input:'+yx+'->Root word: '+otstem+"->Turkum: NOUN"+'->Suffix:'+diffot+"->Analiz:"+analizot)
        if len(felstem) > 0: 
            root_3.append('Input:'+yx+'->Root word: '+felstem+"->Turkum: VERB"+'->Suffix:'+difffel+"->Analiz:"+analizfel)    
