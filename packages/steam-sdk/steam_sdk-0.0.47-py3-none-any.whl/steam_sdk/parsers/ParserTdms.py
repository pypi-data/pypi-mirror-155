...
class ParserTdms:
    ...
    def writeTdmsToCsv(self, path_output: str, dictionary: {}):
        """
    ...
    def convertTdmsToCsv(self, path_output: str, selected_groups: list = []):
        '''
        ...
        :param path_output: Full path of the output file(s). Note: for each group, a different suffix will be added to the file name
        :param selected_groups: List of groups to convert. If left empty, all groups will be converted
        :return:
        '''
        ...
        for group in selected_groups:
            ...
            full_path_file_group = path_output.split('.csv')[0] + suffix_group + '.csv'

            # Write file
            self.dict_groups_signals[group].to_csv(full_path_file_group, index=False, sep=',')  # , float_format='%.7f')  # Write file
            print('File {} written.'.format(full_path_file_group))
