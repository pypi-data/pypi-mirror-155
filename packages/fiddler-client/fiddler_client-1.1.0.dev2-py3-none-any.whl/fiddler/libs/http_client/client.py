...
class Client(object):
    ...
    def __init__(
        ...
        """
        :param host: Base URL for the api. (e.g. https://fiddler.ai)
        :type host:  string
        ...
        if name in self.methods:
            ...
            def http_request(
                ...
                **_
                ...
                _logger.debug(
                    '{method} Request: {url}'.format(method=method, url=request.url)
                )
                if request.data:
                    _logger.debug('PAYLOAD: {data}'.format(data=request.data))
                _logger.debug('HEADERS: {headers}'.format(headers=request.headers))

                response = Response(self._make_request(request, timeout=timeout))
