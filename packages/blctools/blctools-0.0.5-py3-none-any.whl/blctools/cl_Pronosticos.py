...
class Pronosticos(UsuarioCammesa):
    ...
    def _consultar_pronostico(self,id_parque,nombre_parque=None):
        '''
    ...
    def consultar_pronosticos(self,id_parques):

        self.__check_id_parques(id_parques)
        
        #Listado de IDs, sin nombre
        if isinstance(id_parques,(list,tuple,set)):
            descarga = lambda id: self._consultar_pronostico(int(id))
            lista_dfs = [descarga(v) for v in id_parques]
        
        #Listado de IDs, con nombre (diccionario)
        elif isinstance(id_parques,dict):
            descarga = lambda nombre,id: self._consultar_pronostico(int(id),nombre)
            lista_dfs = [descarga(k,v) for k, v in id_parques.items()]
            
        #Si pusieron un número en formato string y con cero: ejemplo 781.0
        else:
            id_parques = int(float(id_parques))
            self._consultar_pronostico(id_parques)
