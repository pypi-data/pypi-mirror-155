...
def earthquake_distance(place):    
    ...
    json_url = urlopen('https://earthquake.usgs.gov/fdsnws/event/1/query?format=geojson&starttime='+ str(yesterday) +'&endtime=' + str(today))
...
def earthquake_time(place):
    ...
    ts = int(time_v) / 1000
    abc = datetime.utcfromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
    #dt_obj = datetime..strftime('%d-%m-%y')
...
def earthquake_less_than(place,less_than):    
    ...
    rslt_df = df[df['distance'] < less_than]
    rslt_df = rslt_df.sort_values(by=['distance'])
