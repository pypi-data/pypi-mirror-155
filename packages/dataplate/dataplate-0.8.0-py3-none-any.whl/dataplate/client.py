...
class DataPlate:
    ...
    def query(self,
        ...
        es_index_type: str
            elasticSearch option - dynamically add/change the dataset es.resource (index/type) for the allowed cluster [e.g.: index1/type1,index2/type2] ,to search for all types in index ignore the type name (default: None)
        es_node: str
            elasticSearch option - dynamically change host/cluster/node name - use this in case you've defined a regex based es.nodes in the source url of the dataset
        bucket_suffixes: str
    ...
    def query_to_df(self,
        ...
        **kwargs : params
        ...
        with tempfile.NamedTemporaryFile(suffix='.gz') as t:
            output_file = t.name
            self.query(query, output_file, refresh, async_m, request_timeout, es_index_type, es_node, bucket_suffixes, bucket_filter, force_scheme_change, recursive_lookup_no_partitions)
            with open(output_file, 'rb') as fh:
                return pd.read_json(fh, compression='gzip', lines=True, **kwargs)
